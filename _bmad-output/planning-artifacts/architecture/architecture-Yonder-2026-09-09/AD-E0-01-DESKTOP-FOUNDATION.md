...
状态：Proposed（待评审，不得据此转 Accepted）
日期：2026-09-10；最近复核：2026-09-22
...
### 2026-09-22 证据复核与阶段结论

按现有独立证据分层复核：macOS 已通过启动就绪、原生托盘/退出清理、同进程 UDS echo、持续展开资源预算、右侧自然休眠与托盘/键盘/AX 唤醒、普通与全屏 Space 的局部可见性等子范围；四边完整原生休眠、跨用户 Socket 拒绝、完整状态更新延迟、全部生命周期与 Windows 当前版本证据仍缺。Windows 依用户决定暂缓，暂缓不等于通过。

因此 Tauri 2 + Rust Core + `interprocess 2.x` 仅继续作为候选桌面基础栈，并允许已通过的 macOS 功能子范围作为后续研发的阶段事实；本 ADR 仍不得转 Accepted，E0-S1/DS-S1 也不得 Done/Archive。接受条件保持为双平台当前版本对等证据、剩余 macOS 缺口关闭和独立 Verification Goal 通过。
