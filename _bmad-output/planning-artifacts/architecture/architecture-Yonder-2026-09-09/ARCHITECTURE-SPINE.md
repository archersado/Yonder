...
2026-09-20圈选提交附件候选见Proposed [AD-CX-02](AD-CX-02-SESSION-INPUT-ATTACHMENT.md)：在同一已认证`AgentSession`内以有界分块暂存附件，`agent.input`只引用会话内附件标识；不发送本机路径、不扩大单帧、不持久化截图。统一清理Spike通过前不得修改产品协议或启用发送。
