...
guard (2...3).contains(CommandLine.arguments.count), AXIsProcessTrusted(),
      let app = NSRunningApplication.runningApplications(withBundleIdentifier:"com.yonder.desktop").first else { exit(2) }
let named = CommandLine.arguments.contains("--named")
...
if named {
 guard find(original,"整理桌面测试") != nil,find(original,"检查文档测试") != nil,
       let card=findButton(original,"整理桌面测试"),AXUIElementPerformAction(card,"AXPress" as CFString) == .success else {exit(10)}
 let detailEnd=ProcessInfo.processInfo.systemUptime+3
 while find(original,"任务 ID")==nil && ProcessInfo.processInfo.systemUptime<detailEnd {Thread.sleep(forTimeInterval:0.1)}
 guard find(original,"任务 ID") != nil,find(original,"整理桌面测试") != nil else {exit(12)}
}
let capture=Process();capture.executableURL=URL(fileURLWithPath:"/usr/sbin/screencapture");capture.arguments=["-x","-l",String(id),output.appendingPathComponent("native-agent-tasks.png").path];try capture.run();capture.waitUntilExit()
...
guard let outside=candidates.first(where:{!rectangle(petInfo).contains($0) && !rectangle(menu).contains($0)}) else{exit(11)}
move(outside)
...
let report:[String:Any]=["pid":Int(app.processIdentifier),"tasks_hover_opens":hoverOpens,"panel_hover_keeps_open":true,"same_visible_desktop":true,"refresh_operable":true,"leave_hides":true,"real_agent_tasks":true,"agent_names_visible":named,"named_detail_and_task_id_visible":named,"passed":true]
