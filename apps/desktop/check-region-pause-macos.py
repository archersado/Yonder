...
try:
    ...
    if entry == "failure":
        ...
        result = {"entry": entry, "native_passed": native.returncode == 0,
                  ...
                  "lease_retained": "error" in blocked, "events": events[0], "outbox": outbox[0], "sequence": sequence,
                  "atomic_history": events == (sequence, sequence) and outbox == (sequence, sequence),
                  "recording_started": False, "focus_started": control is not None and control[2] is not None, "passed": False}
        result["passed"] = result["native_passed"] and result["task_status"] == "running" and result["control_is_pause_pending"] and result["selection_overlay_opened"] is False and result["lease_retained"] and result["atomic_history"] and not result["focus_started"]
