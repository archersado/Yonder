...
func findButton(_ element:AXUIElement,_ title:String,_ depth:Int=0)->AXUIElement? {
 guard depth<16 else{return nil}
 if ["AXButton","AXCheckBox","AXToggleButton"].contains(attr(element,"AXRole") as? String ?? ""), ["AXTitle","AXDescription","AXValue"].contains(where:{(attr(element,$0) as? String)?.contains(title)==true}) {return element}
...
while !(attr(ax,"AXWindows") as? [AXUIElement] ?? []).contains(where:{attr($0,"AXTitle") as? String == "Yonda"}) && ProcessInfo.processInfo.systemUptime<end {Thread.sleep(forTimeInterval:0.1)}
...
if !hoverOpens {
 guard let extras=attr(ax,"AXExtrasMenuBar"),CFGetTypeID(extras)==AXUIElementGetTypeID(),let show=find(extras as! AXUIElement,"任务总览"),AXUIElementPerformAction(show,"AXPress" as CFString) == .success else{exit(4)}
}
...
let menuReadyEnd=ProcessInfo.processInfo.systemUptime+5
while window().flatMap{findButton($0,"全部")}==nil && ProcessInfo.processInfo.systemUptime<menuReadyEnd {Thread.sleep(forTimeInterval:0.1)}
print("all_button_ready=\(window().flatMap{findButton($0,"全部")} != nil)")
guard let shown=window(),let allTasks=findButton(shown,"全部"),AXUIElementPerformAction(allTasks,"AXPress" as CFString) == .success else{exit(10)}
...
print("takeover_button=\(findButton(original,"接管") != nil) cancel_button=\(findButton(original,"取消任务") != nil)")
guard let takeover=findButton(original,"接管"),attr(takeover,"AXEnabled") as? Bool == false,findButton(original,"取消任务") != nil,let all=findButton(original,"全部"),AXUIElementPerformAction(all,"AXPress" as CFString) == .success else{exit(10)}
let allEnd=ProcessInfo.processInfo.systemUptime+5
while find(original,"已取消")==nil && ProcessInfo.processInfo.systemUptime<allEnd {Thread.sleep(forTimeInterval:0.1)}
guard find(original,"已取消") != nil,find(original,"删除任务")==nil else{exit(11)}
let capture=Process();capture.executableURL=URL(fileURLWithPath:"/usr/sbin/screencapture");capture.arguments=["-x","-l",String(id),output.appendingPathComponent("native-card-actions.png").path];try capture.run();capture.waitUntilExit()
...
let candidates=[CGPoint(x:display.minX+5,y:display.minY+30),CGPoint(x:display.maxX-5,y:display.maxY-5),CGPoint(x:display.minX+5,y:display.maxY-5),CGPoint(x:display.maxX-5,y:display.minY+30)]
let petRect=rectangle(info("Yonda")!),menuRect=rectangle(visibleWindow()!)
guard let outside=candidates.first(where:{!petRect.contains($0) && !menuRect.contains($0)}) else{exit(12)}
print("outside_both_windows=true")
move(outside)
...
let report:[String:Any]=["pid":Int(app.processIdentifier),"tasks_hover_opens":hoverOpens,"history_via_tray":!hoverOpens,"panel_hover_keeps_open":true,"same_visible_desktop":true,"refresh_operable":true,"leave_hides":true,"real_agent_tasks":true,"card_actions_present":true,"takeover_disabled_before_execution":true,"cancel_keeps_history":true,"no_data_delete_action":true,"passed":true]
