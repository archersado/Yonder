//! 正式小龙窗口展示；任务事实由唯一TaskHost读取，不保存或创建任务。
use std::sync::Mutex;
...
impl Default for PetWindowState {
    fn default() -> Self {
        Self { restore: Mutex::new(None) }
    }
}

fn can_dock(activity: yonder_application::ActivityState) -> bool {
    activity == yonder_application::ActivityState::NoKnownWork
}
...
#[tauri::command]
pub fn pet_dock(window: WebviewWindow, state: State<'_, PetWindowState>, host: State<'_, crate::TaskState>) -> Result<&'static str, String> {
    if window.label() != "pet" { return Err("不允许的窗口".into()); }
    let mut host = host.0.lock().map_err(|_| "任务状态不可用")?;
    let host = host.as_mut().ok_or("任务状态未知，保持展开")?;
    if !can_dock(host.activity()) {
        return Err("后台任务运行中或状态未知，保持展开".into());
    }
...
mod tests {
    ...
    fn task_gate_and_monitor_bounds() {
        assert!(can_dock(yonder_application::ActivityState::NoKnownWork));
        assert!(!can_dock(yonder_application::ActivityState::Busy));
        assert!(!can_dock(yonder_application::ActivityState::Unknown));
