...
(() => {
  ...
  regionTrigger.addEventListener('click', event => { event.stopPropagation(); interact(); native('region_preview_open').catch(error => { regionStatus.textContent=String(error?.message??error).includes('desktop-control-active')?'Agent 正在控制桌面，暂不能圈选。':'圈选暂不可用，请稍后重试。'; }); });
  async function hide() {
