...
(() => {
  ...
  regionTrigger.addEventListener('click', event => { event.stopPropagation(); interact(); regionStatus.textContent='正在暂停当前任务…'; native('region_preview_open').catch(error => { regionStatus.textContent=String(error?.message??error).includes('desktop-stop-unconfirmed')?'当前任务结果待核实，暂不能圈选。':'圈选暂不可用，请稍后重试。'; }); });
  async function hide() {
