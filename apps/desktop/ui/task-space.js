...
async function select(task, button) {
  ...
  try {
    ...
    const waitReason = timelineResult.status === 'fulfilled' && timelineResult.value.kind === 'events'
      ? [...timelineResult.value.events].reverse().find(event => typeof event.wait_reason === 'string' && event.wait_reason)?.wait_reason ?? '未提供'
      : '未提供';
    ...
    for (const [name, value] of [['任务 ID', result.task.task_id], ['Agent', result.task.owner_agent_id], ['状态', labels[result.task.status] ?? result.task.status], ['序号', result.task.sequence], ['Agent 当前步骤', result.step?.label ?? 'Agent 尚未声明步骤'], ['步骤标识', result.step ? `${result.step.step_id} · 接受序号 ${result.step.accepted_sequence}` : '未提供'], ['等待原因', waitReason], ['浏览器 Task Space', browser]]) {
