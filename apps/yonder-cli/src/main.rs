...
fn agent_id() -> io::Result<String> {
    ...
    if yonder_protocol::valid_id(&id) { Ok(id) } else { Err(io::Error::other("YONDER_AGENT_ID无效")) }
}
...
async fn gateway(agent_id: &str, request: Request) -> io::Result<Response> {
...
fn request(name: &str, args: &Value, id: String, deadline: u64, agent_id: &str) -> Result<Request, String> {
...
async fn handle(message: Value, counter: &mut u64, agent_id: &str) -> Option<Value> {
    ...
    let result = match method {
        ...
        "tools/call" => {
            *counter += 1;
            ...
            match now_ms().map_err(|e| e.to_string()).and_then(|now| request(name, args, format!("mcp-{counter}"), now + 60_000, agent_id)) {
                Ok(request) => match gateway(agent_id, request).await {
...
mod tests {
    ...
    fn mcp_tools_build_typed_gateway_requests() {
        ...
        assert!(matches!(create_request, Request::Create { params, .. } if params.agent_id == "agent-a"));
        assert!(matches!(request("task_control",&json!({"task_id":"task-1","expected_sequence":"4","kind":"takeover"}),"r2".into(),2000,"agent-a").unwrap(),Request::Control { params:ControlParams { kind:ControlKind::Takeover,.. },.. }));
