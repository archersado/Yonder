...
fn source_name(value: TaskSource) -> &'static str {
    match value {
        ...
        TaskSource::CloudAgent => "cloud-agent",
        TaskSource::Legacy => "legacy",
    }
}

fn task_source(value: &str) -> Result<TaskSource, Error> {
    match value {
        "local-agent" => Ok(TaskSource::LocalAgent),
        "cloud-agent" => Ok(TaskSource::CloudAgent),
        "legacy" => Ok(TaskSource::Legacy),
        _ => Err(Error::StorageUnavailable),
    }
}

fn observation_result_name(value: TaskObservationResult) -> &'static str {
    match value {
        TaskObservationResult::Matched => "matched",
        TaskObservationResult::NotMatched => "not-matched",
        TaskObservationResult::Unknown => "unknown",
    }
}

fn observation_result(value: &str) -> Result<TaskObservationResult, Error> {
    match value {
        "matched" => Ok(TaskObservationResult::Matched),
        "not-matched" => Ok(TaskObservationResult::NotMatched),
...
fn status(value: &str) -> Result<Status, Error> {
    match value {
        ...
        "waiting-for-user" => Ok(Status::WaitingForUser),
...
fn parse_reason(value: &str) -> Result<UnknownReason, Error> {
    match value {
        "invalid-input" => Ok(UnknownReason::InvalidInput),
        "dependency-unavailable" => Ok(UnknownReason::DependencyUnavailable),
        "worker-failed" => Ok(UnknownReason::WorkerFailed),
        "timed-out" => Ok(UnknownReason::TimedOut),
        "invalid-response" => Ok(UnknownReason::InvalidResponse),
        "identity-mismatch" => Ok(UnknownReason::IdentityMismatch),
        "observe-failed" => Ok(UnknownReason::ObserveFailed),
        "user-input" => Ok(UnknownReason::UserInput),
        _ => Err(Error::StorageUnavailable),
    }
}

fn focus_failure(value: FocusFailure) -> &'static str {
    match value {
        FocusFailure::PermissionUnavailable => "permission-unavailable",
        FocusFailure::ProcessChanged => "process-changed",
        FocusFailure::WindowMissing => "window-missing",
        FocusFailure::MappingNotUnique => "mapping-not-unique",
        FocusFailure::ActivationFailed => "activation-failed",
        FocusFailure::VerificationFailed => "verification-failed",
        FocusFailure::GeometryChanged => "geometry-changed",
        FocusFailure::ReferenceUnavailable => "reference-unavailable",
    }
}
fn parse_focus_failure(value: &str) -> Result<FocusFailure, Error> {
    match value {
        "permission-unavailable" => Ok(FocusFailure::PermissionUnavailable),
        "process-changed" => Ok(FocusFailure::ProcessChanged),
        "window-missing" => Ok(FocusFailure::WindowMissing),
        "mapping-not-unique" => Ok(FocusFailure::MappingNotUnique),
        "activation-failed" => Ok(FocusFailure::ActivationFailed),
...
impl SqliteTaskStore {
    ...
    pub fn open(path: &Path, database_key: &[u8; 32]) -> Result<Self, Error> {
        ...
        let result =
            unsafe { rusqlite::ffi::sqlite3_key(db.handle(), database_key.as_ptr().cast(), 32) };
    ...
    fn initialize(mut db: Connection, migrate_plaintext: bool) -> Result<Self, Error> {
        ...
        if schema != 0 && schema != 15 {
            ...
            if let Some(path) = db.path().filter(|path| !path.is_empty()) {
                let nonce = std::time::SystemTime::now()
                    ...
                    .as_nanos();
                let backup = format!(
                    "{path}.pre-attempt-v{schema}-{}-{nonce}.db",
                    std::process::id()
                );
        ...
        if schema == 0 || schema == 2 {
            tx.execute_batch(include_str!("task_registration_schema.sql"))
                .map_err(storage)?;
        }
        if schema == 4 {
        ...
        if schema < 5 {
            tx.execute_batch(include_str!("task_name_schema.sql"))
                .map_err(storage)?;
        }
        if schema < 6 {
            tx.execute_batch(include_str!("task_step_schema.sql"))
                .map_err(storage)?;
        }
        if schema < 7 {
            tx.execute_batch(include_str!("task_attempt_schema.sql"))
                .map_err(storage)?;
        }
        if schema < 8 {
            tx.execute_batch(include_str!("task_attempt_result_schema.sql"))
                .map_err(storage)?;
        }
        if schema < 9 {
            tx.execute_batch(include_str!("task_stop_schema.sql"))
                .map_err(storage)?;
        }
        if schema < 10 {
            tx.execute_batch(include_str!("task_control_schema.sql"))
                .map_err(storage)?;
        }
        if schema < 11 {
            tx.execute_batch(include_str!("task_advance_schema.sql"))
                .map_err(storage)?;
        }
        if schema < 12 {
            tx.execute_batch(include_str!("task_browser_reference_schema.sql"))
                .map_err(storage)?;
        }
        if schema < 13 {
            tx.execute_batch(include_str!("task_focus_schema.sql"))
                .map_err(storage)?;
        }
        if schema < 14 {
            tx.execute_batch(include_str!("task_wait_schema.sql"))
                .map_err(storage)?;
        }
        if schema < 15 {
    ...
    fn update_focus(
        ...
        task_id: &str,
        control_id: &str,
        result: Option<Option<FocusFailure>>,
    ) -> Result<(Task, ControlRequestRecord), Error> {
        if !yonder_application::valid_id(task_id) || !yonder_application::valid_id(control_id) {
        ...
        let tx = self
            ...
            .transaction_with_behavior(TransactionBehavior::Immediate)
            .map_err(storage)?;
        let (state, sequence): (String, i64) = tx
            .query_row(
        ...
        if kind != "takeover" || phase != "stopped" {
            return Err(Error::StopRequired);
        }
        let (next, failure) = match result {
            None => ("locating", None),
            Some(None) => ("focused", None),
            Some(Some(value)) => ("failed", Some(focus_failure(value))),
        };
        if current.as_deref() == Some(next) && current_failure.as_deref() == failure {
            tx.commit().map_err(storage)?;
            let task = TaskStore::get(self, task_id)?;
            let control =
                TaskStore::get_control(self, task_id)?.ok_or(Error::StorageUnavailable)?;
            return Ok((task, control));
        }
        if (result.is_none() && current.is_some())
            || (result.is_some() && current.as_deref() != Some("locating"))
        {
        ...
        if tx
            .execute(
                "UPDATE tasks SET sequence=?1 WHERE id=?2 AND state='paused' AND sequence=?3",
        ...
        tx.execute(
            "INSERT INTO events(task_id,sequence,previous,state) VALUES (?1,?2,'paused','paused')",
        ...
        tx.commit().map_err(storage)?;
        let task = TaskStore::get(self, task_id)?;
        let control = TaskStore::get_control(self, task_id)?.ok_or(Error::StorageUnavailable)?;
        Ok((task, control))
    }
}

impl TaskStore for SqliteTaskStore {
    fn supports_pending_cancel(&self) -> bool {
        true
    }
    ...
    fn supports_browser_references(&self) -> bool {
        true
    }
    fn supports_controls(&self) -> bool {
        true
    }
    fn supports_wait_for_user(&self) -> bool {
        true
    }
    ...
    fn wait_for_user(&mut self, task_id: &str, expected: u64, reason: &str) -> Result<Task, Error> {
        ...
        let row: Option<(String, i64, String, Option<String>, String)> = tx
            ...
            .optional()
            .map_err(storage)?;
        let (state, sequence, owner_agent_id, task_name, source) = row.ok_or(Error::NotFound)?;
        if state != "running" || sequence != expected as i64 {
            return Err(Error::Conflict);
        }
        let pending: i64 = tx
            .query_row(
        ...
        if pending != 0 || phase.as_deref() != Some("stopped") {
            return Err(Error::StopRequired);
        }
        let next = expected + 1;
    ...
    fn prepare_attempt(
        &mut self,
        requested: &ExecutionAttempt,
        expected: u64,
    ) -> Result<(Task, ExecutionAttempt), Error> {
        if expected == 0
            || expected >= i64::MAX as u64
            || requested.phase != AttemptPhase::Prepared
            || requested.accepted_sequence != 0
            || [
                requested.task_id.as_str(),
                requested.step_id.as_str(),
                requested.attempt_id.as_str(),
                requested.worker_instance_id.as_str(),
                requested.host_session_id.as_str(),
            ]
            .iter()
            .any(|id| !yonder_application::valid_id(id))
        {
            return Err(Error::InvalidInput);
        }
        let tx = self
        ...
        if let Some((step_id, attempt_id, worker_instance_id, host_session_id, phase, accepted)) =
            existing
        {
            let accepted_sequence =
                u64::try_from(accepted).map_err(|_| Error::StorageUnavailable)?;
            if phase != "prepared"
                || step_id != requested.step_id
                || worker_instance_id != requested.worker_instance_id
                || host_session_id != requested.host_session_id
                || accepted_sequence != expected + 1
            {
                return Err(Error::Conflict);
            }
            tx.commit().map_err(storage)?;
            return Ok((
                task,
                ExecutionAttempt {
                    task_id: requested.task_id.clone(),
                    step_id,
                    attempt_id,
                    worker_instance_id,
                    host_session_id,
        ...
        if !matches!(task.status, Status::Created | Status::Running) || task.sequence != expected {
            return Err(Error::Conflict);
        }
        ...
        if current_step.as_ref().map(|value| value.0.as_str()) != Some(requested.step_id.as_str()) {
            return Err(Error::StopRequired);
        }
        ...
        tx.execute(
            ...
            params![requested.task_id, accepted as i64, state],
        )
        .map_err(storage)?;
        tx.execute(
            "INSERT INTO outbox(task_id,sequence) VALUES (?1,?2)",
            params![requested.task_id, accepted as i64],
        )
        .map_err(storage)?;
        ...
        tx.commit().map_err(storage)?;
        let accepted_attempt = ExecutionAttempt {
        ...
        row.map(
            |(
                ...
                let conclusion = match (phase.as_str(), action, observe, reason_value.as_deref()) {
                    ("observed" | "stopped", Some(value), Some(1), None) => {
                        AttemptConclusion::Observed {
                            action_succeeded: value != 0,
                        }
                    }
                    ("unknown", None, Some(0), Some(value)) => AttemptConclusion::Unknown {
                        reason: parse_reason(value)?,
                    },
                    _ => return Err(Error::StopRequired),
                };
                Ok(AttemptResultRecord {
                    task_id: task_id.into(),
                    step_id,
                    attempt_id,
                    worker_instance_id,
                    host_session_id,
                    conclusion,
                    result_sequence: u64::try_from(result.ok_or(Error::StorageUnavailable)?)
                        .map_err(|_| Error::StorageUnavailable)?,
                })
            },
        )
    ...
    fn record_attempt_result(
        ...
        if expected == 0
            || expected >= i64::MAX as u64
            || attempt.accepted_sequence == 0
            || [
                attempt.task_id.as_str(),
                attempt.step_id.as_str(),
                attempt.attempt_id.as_str(),
                attempt.worker_instance_id.as_str(),
                attempt.host_session_id.as_str(),
            ]
            .iter()
            .any(|id| !yonder_application::valid_id(id))
        {
        ...
        let tx = self
            ...
            .transaction_with_behavior(TransactionBehavior::Immediate)
            .map_err(storage)?;
        let task_row: Option<(String, i64, String, Option<String>, String)> = tx
            .query_row(
                "SELECT state,sequence,owner_agent_id,name,source FROM tasks WHERE id=?1",
                [&attempt.task_id],
                |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?, r.get(3)?, r.get(4)?)),
            ...
            .optional()
            .map_err(storage)?;
        let (task_state, task_sequence, owner, name_value, source) =
            task_row.ok_or(Error::NotFound)?;
        let task = Task {
            id: attempt.task_id.clone(),
            owner_agent_id: owner,
            name: name_value,
            source: task_source(&source)?,
            status: status(&task_state)?,
            sequence: u64::try_from(task_sequence).map_err(|_| Error::StorageUnavailable)?,
        };
        if task.status != Status::Running || task.sequence != expected {
            return Err(Error::Conflict);
        }
        ...
        let (
            ...
            observe_valid,
            unknown_reason,
        ) = row.ok_or(Error::Conflict)?;
        if step_id != attempt.step_id
            || worker_id != attempt.worker_instance_id
            || host_id != attempt.host_session_id
            || u64::try_from(accepted).map_err(|_| Error::StorageUnavailable)?
        ...
        if phase != "prepared" {
            ...
            let existing = match (
                ...
                ("observed", Some(value), Some(1), None) => AttemptConclusion::Observed {
                    action_succeeded: value != 0,
                },
                ("unknown", None, Some(0), Some(value)) => AttemptConclusion::Unknown {
                    reason: parse_reason(value)?,
            ...
            if existing != conclusion || result_sequence != Some(task_sequence) {
                return Err(Error::Conflict);
            }
            tx.commit().map_err(storage)?;
            return Ok((
                ...
                AttemptResultRecord {
                    ...
                    conclusion: existing,
                    result_sequence: u64::try_from(task_sequence)
                        .map_err(|_| Error::StorageUnavailable)?,
                },
            ));
        }
        ...
        let (phase_name, action_value, observe_value, reason_value) = match conclusion {
            AttemptConclusion::Observed { action_succeeded } => {
                ("observed", Some(i64::from(action_succeeded)), 1, None)
            }
            AttemptConclusion::Unknown { reason: unknown } => {
                ("unknown", None, 0, Some(reason(unknown)))
            }
        };
        if tx
            .execute(
                "UPDATE tasks SET sequence=?1 WHERE id=?2 AND state='running' AND sequence=?3",
                params![sequence as i64, attempt.task_id, expected as i64],
            )
            .map_err(storage)?
            != 1
        {
        ...
        tx.execute(
            "INSERT INTO outbox(task_id,sequence) VALUES (?1,?2)",
            params![attempt.task_id, sequence as i64],
        )
        .map_err(storage)?;
    ...
    fn record_browser_result(
        ...
        attempt: &ExecutionAttempt,
        expected: u64,
        reference: &BrowserReferenceRecord,
    ) -> Result<(Task, AttemptResultRecord), Error> {
        if expected == 0
            || expected >= i64::MAX as u64
            || attempt.accepted_sequence == 0
            || reference.task_id != attempt.task_id
            || !valid_ref(&reference.external_task_ref)
            || !matches!(
                reference.ownership.as_str(),
                "agent" | "agentDelegatedToUser" | "user"
            )
            || reference.managed_pages > 100
            || reference.updated_sequence != 0
        {
            return Err(Error::InvalidInput);
        }
        let tx = self
        ...
        let row: Option<(String, i64, String, Option<String>, String)> = tx
            ...
            .optional()
            .map_err(storage)?;
        let (state, sequence, owner, name_value, source) = row.ok_or(Error::NotFound)?;
        let task = Task {
            id: attempt.task_id.clone(),
            owner_agent_id: owner,
            name: name_value,
        ...
        let (step_id, attempt_id, worker_id, host_id, phase, accepted, result_sequence) =
        ...
        if phase != "prepared" {
            if phase != "observed" || result_sequence != Some(sequence) {
                return Err(Error::Conflict);
            }
            ...
            if !matches!(stored,Some((ref external,ref ownership,pages,finished,updated)) if external==&reference.external_task_ref && ownership==&reference.ownership && usize::try_from(pages).ok()==Some(reference.managed_pages) && (finished!=0)==reference.finished && updated==sequence)
            {
                return Err(Error::Conflict);
            }
            tx.commit().map_err(storage)?;
            return Ok((
                ...
                AttemptResultRecord {
                    ...
                    worker_instance_id: worker_id,
                    host_session_id: host_id,
                    conclusion: AttemptConclusion::Observed {
                        action_succeeded: true,
                    },
                    result_sequence: u64::try_from(sequence)
                        .map_err(|_| Error::StorageUnavailable)?,
                },
            ));
        }
        ...
        if tx
            .execute(
                "UPDATE tasks SET sequence=?1 WHERE id=?2 AND state='running' AND sequence=?3",
                params![next as i64, attempt.task_id, expected as i64],
            )
            .map_err(storage)?
            != 1
        {
        ...
        tx.execute(
            "INSERT INTO outbox(task_id,sequence) VALUES (?1,?2)",
            params![attempt.task_id, next as i64],
        )
        .map_err(storage)?;
    ...
    fn get_browser_reference(
        &mut self,
        task_id: &str,
    ) -> Result<Option<BrowserReferenceRecord>, Error> {
        if !yonder_application::valid_id(task_id) {
            return Err(Error::InvalidInput);
        }
        ...
        row.map(
            |(external_task_ref, ownership, managed_pages, finished, updated_sequence)| {
                Ok(BrowserReferenceRecord {
                    task_id: task_id.into(),
                    external_task_ref,
                    ownership,
                    managed_pages: usize::try_from(managed_pages)
                        .map_err(|_| Error::StorageUnavailable)?,
                    finished: finished != 0,
                    updated_sequence: u64::try_from(updated_sequence)
                        .map_err(|_| Error::StorageUnavailable)?,
                })
            },
        )
    ...
    fn advance_attempt(
        ...
        attempt: &ExecutionAttempt,
        expected: u64,
    ) -> Result<(Task, StepBoundaryRecord), Error> {
        if expected == 0
            || expected >= i64::MAX as u64
            || !matches!(
                attempt.phase,
                AttemptPhase::Observed | AttemptPhase::Stopped
            )
        {
            return Err(Error::InvalidInput);
        }
        ...
        let (step_id, attempt_id, worker_id, host_id, phase, accepted, stopped, control_id) =
            current.ok_or(Error::Conflict)?;
        if step_id != attempt.step_id
            || worker_id != attempt.worker_instance_id
            || host_id != attempt.host_session_id
        ...
        if phase == "stopped" {
            ...
            let stop_sequence = u64::try_from(stopped.ok_or(Error::StorageUnavailable)?)
                .map_err(|_| Error::StorageUnavailable)?;
            tx.commit().map_err(storage)?;
            return Ok((
                task,
    ...
    fn request_control(
        ...
        if expected == 0 || expected >= i64::MAX as u64 || !yonder_application::valid_id(control_id)
        {
            return Err(Error::InvalidInput);
        }
        ...
        )) = existing
        {
            ...
            if attempt_id != attempt.attempt_id
                || stored_id != control_id
                || stored_kind != kind_name
                || (expected != accepted && expected.checked_add(1) != Some(accepted))
            {
                return Err(Error::Conflict);
            }
            let phase = match phase.as_str() {
                "pending" => ControlPhase::Pending,
                "stopped" => ControlPhase::Stopped,
                _ => return Err(Error::StorageUnavailable),
            };
            tx.commit().map_err(storage)?;
            let focus_phase = match focus.as_deref() {
                Some("locating") => Some(FocusPhase::Locating),
                Some("focused") => Some(FocusPhase::Focused),
                Some("failed") => Some(FocusPhase::Failed),
                None => None,
                _ => return Err(Error::StorageUnavailable),
            };
            let focus_failure = focus_error
                .as_deref()
                .map(parse_focus_failure)
                .transpose()?;
            return Ok((
                task,
        ...
        tx.execute(
            "INSERT INTO events(task_id,sequence,previous,state) VALUES (?1,?2,'running',?3)",
            params![attempt.task_id, accepted as i64, next_state],
        )
        .map_err(storage)?;
        tx.execute(
            "INSERT INTO outbox(task_id,sequence) VALUES (?1,?2)",
            params![attempt.task_id, accepted as i64],
        )
        .map_err(storage)?;
        ...
        if direct {
            tx.execute("UPDATE task_attempts SET control_id=?1,control_kind=?2 WHERE task_id=?3 AND attempt_id=?4 AND phase='stopped' AND control_id IS NULL",params![control_id,kind_name,attempt.task_id,attempt.attempt_id]).map_err(storage)?;
        }
        tx.commit().map_err(storage)?;
        Ok((
            Task {
            ...
            ControlRequestRecord {
                ...
                accepted_sequence: accepted,
                stopped_sequence: if direct { Some(accepted) } else { None },
                focus_phase: None,
                focus_failure: None,
            },
        ))
    ...
    fn get_control(&mut self, task_id: &str) -> Result<Option<ControlRequestRecord>, Error> {
        ...
        row.map(
            |(attempt_id, control_id, kind, phase, accepted, stopped, focus, focus_error)| {
                Ok(ControlRequestRecord {
                    task_id: task_id.into(),
                    attempt_id,
                    control_id,
                    ...
                    stopped_sequence: stopped
                        ...
                        .transpose()
                        .map_err(|_| Error::StorageUnavailable)?,
                    focus_phase: match focus.as_deref() {
                        Some("locating") => Some(FocusPhase::Locating),
                        Some("focused") => Some(FocusPhase::Focused),
                        Some("failed") => Some(FocusPhase::Failed),
                        None => None,
    ...
    fn begin_focus(
        ...
        self.update_focus(task_id, control_id, None)
    }
    fn finish_focus(
        &mut self,
        task_id: &str,
        control_id: &str,
        failure: Option<FocusFailure>,
    ) -> Result<(Task, ControlRequestRecord), Error> {
        self.update_focus(task_id, control_id, Some(failure))
    }
    ...
    fn stop_attempt(
        ...
        let action = match kind {
            ControlKind::Pause | ControlKind::Takeover => Action::Pause,
            ControlKind::Cancel => Action::Cancel,
        };
        let tx = self
            .0
            .transaction_with_behavior(TransactionBehavior::Immediate)
        ...
        if control
            .as_ref()
            .is_none_or(|(attempt_id, stored_kind, phase)| {
                attempt_id != &attempt.attempt_id
                    || stored_kind != kind_name
                    || !matches!(phase.as_str(), "pending" | "stopped")
            })
        {
            return Err(Error::StopRequired);
        }
        ...
        if phase == "stopped" {
            ...
            let expected_status = if kind == ControlKind::Cancel {
                Status::Cancelled
            } else {
                Status::Paused
            };
            if stored_control.as_deref() != Some(control_id)
                || stored_kind.as_deref() != Some(kind_name)
                || stop_sequence != task_sequence
                || task.status != expected_status
                || control
                    .as_ref()
                    .is_none_or(|(_, _, phase)| phase != "stopped")
            {
                return Err(Error::Conflict);
            }
            tx.commit().map_err(storage)?;
            return Ok((
                ...
                StopRecord {
                    ...
                    control_id: control_id.into(),
                    kind,
                    stop_sequence: u64::try_from(stop_sequence)
                        .map_err(|_| Error::StorageUnavailable)?,
                },
            ));
        }
        if phase != "observed" || action_succeeded.is_none() || task.status != Status::Running {
            return Err(Error::StopRequired);
        }
        let transition = task
            .status
            .transition(expected, action)
            .map_err(Error::InvalidTransition)?;
        ...
        tx.execute(
            "INSERT INTO outbox(task_id,sequence) VALUES (?1,?2)",
            params![attempt.task_id, transition.sequence as i64],
        )
        .map_err(storage)?;
        ...
        Ok((
            ...
            StopRecord {
                ...
                control_id: control_id.into(),
                kind,
                stop_sequence: transition.sequence,
            },
        ))
    }
    ...
    fn declare_step(
        ...
        step_id: &str,
        label: &str,
    ) -> Result<(Task, StepDeclaration), Error> {
        if [owner, task_id, step_id]
            .iter()
            .any(|id| !yonder_application::valid_id(id))
            || expected == 0
            || expected >= i64::MAX as u64
            || !yonder_application::valid_step_label(label)
        {
            return Err(Error::InvalidInput);
        }
        let tx = self
        ...
        let row: Option<(String, i64, String, Option<String>, String)> = tx
            ...
            .optional()
            .map_err(storage)?;
        let (state, sequence, actual_owner, task_name, source) = row.ok_or(Error::NotFound)?;
        if actual_owner != owner {
            return Err(Error::NotFound);
        }
        let current = Task {
            id: task_id.into(),
            owner_agent_id: actual_owner,
            name: task_name,
            source: task_source(&source)?,
            status: status(&state)?,
            sequence: u64::try_from(sequence).map_err(|_| Error::StorageUnavailable)?,
        ...
        let existing: Option<(String, i64)> = tx
            .query_row(
                "SELECT label,accepted_sequence FROM task_steps WHERE task_id=?1 AND step_id=?2",
        ...
        if !matches!(current.status, Status::Created | Status::Running) {
            return Err(Error::StopRequired);
        }
        ...
        let total_count: i64 = tx
            ...
            .map_err(storage)?;
        if task_count >= 1024 || total_count >= 10000 {
        ...
        tx.execute(
            ...
            params![task_id, accepted as i64, state],
        )
        .map_err(storage)?;
        tx.execute(
            "INSERT INTO outbox(task_id,sequence) VALUES (?1,?2)",
            params![task_id, accepted as i64],
        )
        .map_err(storage)?;
        tx.execute(
            "INSERT INTO task_steps(task_id,step_id,label,accepted_sequence) VALUES (?1,?2,?3,?4)",
            params![task_id, step_id, label, accepted as i64],
        )
        .map_err(storage)?;
        tx.commit().map_err(storage)?;
        Ok((
    ...
    fn get_with_step(&mut self, id: &str) -> Result<(Task, Option<StepDeclaration>), Error> {
        ...
        let row: Option<(String, i64, String, Option<String>, String)> = tx
            ...
            .optional()
            .map_err(storage)?;
        let (state, sequence, owner_agent_id, name, source) = row.ok_or(Error::NotFound)?;
        ...
        tx.commit().map_err(storage)?;
        let step = match step {
            Some((step_id, label, accepted)) => Some(StepDeclaration {
                step_id,
                label,
                accepted_sequence: u64::try_from(accepted)
                    .map_err(|_| Error::StorageUnavailable)?,
    ...
    fn get_presentation(&mut self, id: &str) -> Result<(Task, TaskPresentation), Error> {
        let tx = self.0.transaction().map_err(storage)?;
        let row: Option<(String,i64,String,Option<String>,String,Option<String>,Option<String>,Option<String>,Option<String>)> = tx.query_row(
            "SELECT state,sequence,owner_agent_id,name,source,observation_step_id,observation_result,observation_summary,next_intent FROM tasks WHERE id=?1", [id],
        ...
        let observation = match (
            ...
            (Some(step_id), Some(result), Some(summary)) => Some(TaskObservation {
                step_id,
    ...
    fn record_observation(
        ...
        let attempt: Option<(String,String,Option<i64>,Option<String>)> = tx.query_row(
            "SELECT step_id,phase,action_succeeded,unknown_reason FROM task_attempts WHERE task_id=?1 AND attempt_id=?2 AND accepted_sequence=(SELECT max(accepted_sequence) FROM task_attempts WHERE task_id=?1)",
            params![task_id,attempt_id], |r| Ok((r.get(0)?,r.get(1)?,r.get(2)?,r.get(3)?)),
        ).optional().map_err(storage)?;
        let (step_id, phase, action_succeeded, unknown_reason) = attempt.ok_or(Error::Conflict)?;
        if matches!(phase.as_str(), "observed") && action_succeeded.is_none()
            || matches!(phase.as_str(), "unknown")
                && (result != TaskObservationResult::Unknown || unknown_reason.is_none())
            || !matches!(phase.as_str(), "observed" | "unknown")
        {
            return Err(Error::Conflict);
        }
        let next = expected + 1;
        if tx.execute("UPDATE tasks SET sequence=?1,observation_step_id=?2,observation_result=?3,observation_summary=?4 WHERE id=?5 AND state='running' AND sequence=?6", params![next as i64,step_id,observation_result_name(result),summary,task_id,expected as i64]).map_err(storage)? != 1 { return Err(Error::Conflict); }
        ...
        .map_err(storage)?;
        let payload = format!(
            r#"{{"step_id":"{step_id}","result":"{}","summary":{}}}"#,
            observation_result_name(result),
            serde_json::to_string(summary).map_err(|_| Error::StorageUnavailable)?
        );
        tx.execute("INSERT INTO task_presentation_events(task_id,sequence,kind,payload) VALUES (?1,?2,'observation',?3)", params![task_id,next as i64,payload]).map_err(storage)?;
        tx.commit().map_err(storage)?;
        Ok(Task {
            sequence: next,
            ..task
    ...
    fn declare_next_intent(
        ...
        let row: Option<(String, i64, String, Option<String>, String)> = tx
            ...
            .map_err(storage)?;
        let (state, sequence, owner_agent_id, name, source) = row.ok_or(Error::NotFound)?;
        if owner.is_some_and(|owner| owner != owner_agent_id) {
            return Err(Error::NotFound);
        }
        let task = Task {
            id: task_id.into(),
        ...
        let next = expected + 1;
        if tx.execute("UPDATE tasks SET sequence=?1,next_intent=?2 WHERE id=?3 AND sequence=?4 AND state NOT IN ('completed','failed','cancelled')", params![next as i64,intent,task_id,expected as i64]).map_err(storage)? != 1 { return Err(Error::Conflict); }
        tx.execute(
            "INSERT INTO events(task_id,sequence,previous,state) VALUES (?1,?2,?3,?3)",
        ...
        .map_err(storage)?;
        tx.execute("INSERT INTO task_presentation_events(task_id,sequence,kind,payload) VALUES (?1,?2,'next-intent',?3)", params![task_id,next as i64,format!(r#"{{"next_intent":{}}}"#, serde_json::to_string(intent).map_err(|_| Error::StorageUnavailable)?) ]).map_err(storage)?;
        tx.commit().map_err(storage)?;
        Ok(Task {
            sequence: next,
            ..task
    ...
    fn events_with_steps(
        &mut self,
        id: &str,
        after: u64,
        limit: usize,
    ) -> Result<Vec<TaskEventRecord>, Error> {
        ...
        rows.map(|row| {
            ...
            let attempt_result = match (
                ...
                (
                    ...
                    let conclusion =
                        match (phase.as_str(), action, observe, reason_value.as_deref()) {
                            ("observed" | "stopped", Some(value), Some(1), None) => {
                                AttemptConclusion::Observed {
                                    action_succeeded: value != 0,
                                }
    ...
    fn register(
        ...
        if [owner, key].iter().any(|id| {
            ...
            || task_name.is_some_and(|name| !yonder_application::valid_task_name(name))
        {
            return Err(Error::InvalidInput);
        }
        let tx = self
            ...
            .transaction_with_behavior(TransactionBehavior::Immediate)
            .map_err(storage)?;
        let existing: Option<(String,String,String,i64,Option<String>,Option<String>,String)> = tx.query_row(
        ...
        )) = existing
        {
            if original != description
                || original_name.as_deref() != task_name
                || current_source != source_name(source)
            {
    ...
    fn list(
        ...
        let mut stmt = self.0.prepare("SELECT id,state,sequence,owner_agent_id,name,source FROM tasks WHERE id > ?1 COLLATE BINARY AND (?2 OR state NOT IN ('completed','failed','cancelled')) AND (?4 IS NULL OR owner_agent_id=?4) ORDER BY id COLLATE BINARY LIMIT ?3").map_err(storage)?;
        let rows = stmt
            .query_map(
                params![after.unwrap_or(""), include_finished, limit as i64, owner],
                |r| {
                    Ok((
                        r.get::<_, String>(0)?,
                        r.get::<_, String>(1)?,
    ...
    fn list_running(
        ...
        let mut stmt = self.0.prepare("SELECT id,state,sequence,owner_agent_id,name,source FROM tasks WHERE id > ?1 COLLATE BINARY AND state='running' AND (?3 IS NULL OR owner_agent_id=?3) ORDER BY id COLLATE BINARY LIMIT ?2").map_err(storage)?;
        let rows = stmt
            .query_map(params![after.unwrap_or(""), limit as i64, owner], |r| {
                Ok((
                    r.get::<_, String>(0)?,
                    r.get::<_, String>(1)?,
                    r.get::<_, i64>(2)?,
    ...
    fn create(
        ...
        if [id, owner_agent_id].iter().any(|id| {
            id.is_empty()
                || id.len() > 128
                || !id
                    .bytes()
    ...
    fn get(&mut self, id: &str) -> Result<Task, Error> {
        let row: Option<(String, i64, String, Option<String>, String)> = self
            .0
            .query_row(
                "SELECT state,sequence,owner_agent_id,name,source FROM tasks WHERE id=?1",
                [id],
    ...
    fn running(&mut self, limit: usize) -> Result<Vec<Task>, Error> {
        ...
        let mut stmt = self.0.prepare("SELECT id,sequence,owner_agent_id,name,source FROM tasks WHERE state='running' ORDER BY id LIMIT ?1").map_err(storage)?;
        let rows = stmt
            ...
            .map_err(storage)?;
        rows.map(|row| {
            let (id, sequence, owner_agent_id, name, source) = row.map_err(storage)?;
            Ok(Task {
                id,
                owner_agent_id,
                name,
    ...
    fn commit(&mut self, id: &str, expected: u64, change: Transition) -> Result<(), Error> {
        ...
        .iter()
        .any(|action| change.previous.transition(expected, *action) == Ok(change));
        if !legal {
            return Err(Error::InvalidInput);
        }
        let tx = self
        ...
        let changed = tx
            .execute(
                "UPDATE tasks SET state=?1,sequence=?2 WHERE id=?3 AND sequence=?4 AND state=?5",
                params![
                    name(change.next),
        ...
        tx.execute(
            "INSERT INTO outbox(task_id,sequence) VALUES (?1,?2)",
            params![id, change.sequence as i64],
        )
        .map_err(storage)?;
        tx.commit().map_err(storage)
    }
...
mod tests {
    ...
    fn agent_names_are_versioned_persistent_idempotent_and_bounded() {
        ...
        let create = |name: Option<&str>| {
            let extra = name
                ...
                .unwrap_or_default();
            format!(
                r#"{{"jsonrpc":"2.0","id":"c","method":"task.create","params":{{"agent_id":"a1","capability":"task.create","deadline":2000,"idempotency_key":"named","description":"test"{extra}}}}}"#
            )
        };
        let failure = |response: Response, code| {
            assert!(matches!(response, Response::Failure { error, .. } if error.code == code))
        };
        session.handle(&mut store, hello(2).as_bytes(), 1000);
        failure(
            session.handle(&mut store, create(Some("整理文件")).as_bytes(), 1000),
            -32010,
        );
        session.handle(&mut store, hello(3).as_bytes(), 1000);
        failure(
            session.handle(&mut store, create(None).as_bytes(), 1000),
            -32602,
        );
        for name in ["", " ", &"龙".repeat(86)] {
            failure(
                session.handle(&mut store, create(Some(name)).as_bytes(), 1000),
                -32602,
            );
        }
        ...
        let task = yonder_application::cancel_pending(
            &mut store,
        ...
        failure(
            session.handle(&mut store, create(Some("其他名称")).as_bytes(), 1000),
            -32009,
        );
        assert!(
            matches!(session.handle(&mut store, create(Some("整理文件")).as_bytes(), 1000), Response::Success { result: QueryResult::Snapshot { task }, .. } if task.name == first.name && task.status == yonder_protocol::TaskStatus::Cancelled)
        );
        let get = format!(
        ...
        for bytes in [get.as_bytes(), list.as_slice()] {
            let encoded =
                String::from_utf8(session.handle_encoded(&mut store, bytes, 1000).unwrap())
                    .unwrap();
    ...
    fn name_migration_backs_up_preserves_legacy_and_rolls_back_failed_ddl() {
        ...
        db.execute_batch(include_str!("task_registration_schema.sql"))
            .unwrap();
        ...
        assert_eq!(
            backup
                .query_row("SELECT description FROM task_creations", [], |r| r
                    .get::<_, String>(0))
                .unwrap(),
            "retained"
        );
        assert_eq!(
            backup
                .query_row("SELECT sum(delivered) FROM outbox", [], |r| r
                    .get::<_, i64>(0))
                .unwrap(),
            1
        );
        drop(backup);
        let named = yonder_application::register(
            &mut store,
        ...
        db.execute_batch(include_str!("task_registration_schema.sql"))
            .unwrap();
        db.execute_batch("ALTER TABLE task_creations ADD COLUMN name TEXT;")
            .unwrap();
        drop(db);
        ...
        assert_eq!(
            db.query_row(
                "SELECT count(*) FROM pragma_table_info('tasks') WHERE name='name'",
                [],
                |r| r.get::<_, i64>(0)
            )
            .unwrap(),
    ...
    fn unused_experimental_format_rolls_back_without_losing_task_data() {
        ...
        .unwrap();
        let cancelled = yonder_application::cancel_pending(
            &mut store,
            AuthContext::LocalUser("desktop"),
            &task.id,
            1,
        ...
        let mut store = SqliteTaskStore::open_unencrypted(&path).unwrap();
        let legacy = Task {
            source: TaskSource::Legacy,
            ..cancelled.clone()
        };
        assert_eq!(store.get(&task.id), Ok(legacy.clone()));
        assert_eq!(store.events(&task.id, 0, 100).unwrap().len(), 2);
        assert_eq!(
            store
                .0
                .query_row(
                    "SELECT count(*) FROM outbox WHERE task_id=?1",
                    [&task.id],
                    |r| r.get::<_, i64>(0)
                )
                .unwrap(),
        ...
        assert_eq!(
            store
                .0
                .query_row(
                    "SELECT description FROM task_creations WHERE task_id=?1",
                    [&task.id],
                    |r| r.get::<_, String>(0)
                )
                .unwrap(),
        ...
        for rejected in [
            ...
            "CREATE TRIGGER retain_kind AFTER INSERT ON events BEGIN SELECT NEW.kind; END;",
        ] {
            let db = Connection::open(&path).unwrap();
            db.execute_batch(experimental).unwrap();
            ...
            let db = Connection::open(&path).unwrap();
            db.execute_batch("UPDATE tasks SET deleted=0; DROP TRIGGER IF EXISTS retain_kind;")
                .unwrap();
            drop(db);
    ...
    fn task_presentation_metadata_is_persistent_versioned_and_version_gated() {
        ...
        use yonder_protocol::{
            ...
            TaskSource as ProtocolTaskSource,
        };
        ...
        let legacy = store.get("legacy").unwrap();
        assert_eq!(legacy.source, TaskSource::Legacy);
        let (_, legacy_presentation) = store.get_presentation("legacy").unwrap();
        assert_eq!(
            legacy_presentation,
            TaskPresentation {
                current_step: None,
        ...
        .unwrap();
        let (task, step) = store
            .declare_step("a1", &task.id, task.sequence, "open-document", "打开文档")
            .unwrap();
        let requested = ExecutionAttempt {
            task_id: task.id.clone(),
            step_id: "open-document".into(),
        ...
        let (running, accepted, _) = start_attempt(
            &mut store,
            &gate,
            &requested,
            task.sequence,
        ...
        .unwrap();
        let observed = yonder_application::record_observation(
            &mut store,
            &task.id,
            &accepted.attempt_id,
            observed.sequence,
        ...
        .unwrap();
        let task = yonder_application::declare_next_intent(
            &mut store,
            AuthContext::Agent("a1"),
            &task.id,
            observed.sequence,
        ...
        assert_eq!(task.source, TaskSource::LocalAgent);
        assert_eq!(task.sequence, 6);
        assert_eq!(
            store.get_presentation(&task.id).unwrap(),
            (
                task.clone(),
                TaskPresentation {
                    current_step: Some(step),
                    observation: Some(TaskObservation {
                        step_id: "open-document".into(),
                        result: TaskObservationResult::Matched,
                        summary: "文档已打开".into()
                    }),
                    next_intent: Some("编辑文档".into()),
                }
            )
        );
        assert_eq!(
            store
                .0
                .query_row(
                    "SELECT source,observation_result,next_intent FROM tasks WHERE id=?1",
                    [&task.id],
                    |r| Ok((
                        r.get::<_, String>(0)?,
        ...
        assert_eq!(store.0.query_row("SELECT (SELECT count(*) FROM events WHERE task_id=?1),(SELECT count(*) FROM outbox WHERE task_id=?1),(SELECT count(*) FROM task_presentation_events WHERE task_id=?1)", [&task.id], |r| Ok((r.get::<_,i64>(0)?, r.get::<_,i64>(1)?, r.get::<_,i64>(2)?))).unwrap(), (6, 6, 3));
        ...
        let list = format!(
            r#"{{"jsonrpc":"2.0","id":"l","method":"task.list","params":{{"agent_id":"a1","capability":"task.read","deadline":2000,"include_finished":true,"limit":20}}}}"#
        );
        for (minor, source, full) in [
            (18, None, false),
            (19, Some(ProtocolTaskSource::LocalAgent), true),
        ] {
            let mut session = GatewaySession::new(AuthContext::Agent("a1"), Platform::Macos);
            session.handle(&mut store, hello(minor).as_bytes(), 1000);
            ...
            assert_eq!(snapshot.current_step.is_some(), full);
            assert_eq!(
                snapshot.observation.as_ref().map(|observation| (
                    observation.step_id.as_str(),
                    observation.result,
            ...
            assert_eq!(snapshot.next_intent.as_deref(), full.then_some("编辑文档"));
            let tasks = match session.handle(&mut store, list.as_bytes(), 1000) {
                Response::Success {
            ...
            assert_eq!(
                (
                    listed.current_step.is_some(),
                    listed.observation.is_some(),
                    listed.next_intent.is_some()
                ),
                (false, false, false)
            );
        }
    ...
    fn pending_cancel_is_authorized_versioned_atomic_and_does_not_resurrect() {
        ...
        .unwrap();
        let request = format!(
            r#"{{"jsonrpc":"2.0","id":"cancel","method":"task.cancel","params":{{"agent_id":"a1","capability":"task.cancel","deadline":2000,"task_id":"{}","expected_sequence":"1"}}}}"#,
            first.id
        );
        let mut session = GatewaySession::new(AuthContext::Agent("a1"), Platform::Macos);
        let failed = |response: Response, code| {
            assert!(matches!(response, Response::Failure { error, .. } if error.code == code))
        };
        ...
        assert_eq!(
            cancel_pending(&mut store, AuthContext::Agent("b2"), &first.id, 1),
            Err(Error::NotFound)
        );
        assert_eq!(
            cancel_pending(&mut store, AuthContext::Agent("a1"), &first.id, 9),
            Err(Error::Conflict)
        );
        ...
        assert_eq!(
            cancel_pending(&mut store, AuthContext::LocalUser("desktop"), &first.id, 1),
            Ok(cancelled.clone())
        );
        assert_eq!(
            cancel_pending(&mut store, AuthContext::Agent("a1"), &first.id, 2),
            Ok(cancelled.clone())
        );
        ...
        assert_eq!(
            transition(&mut store, &first.id, 1, Action::Start),
            Err(Error::Conflict)
        );
    ...
    fn registration_migrates_is_idempotent_isolated_and_atomic() {
        ...
        failed(
            a.handle(&mut store, &create("b2", "same", "说明"), 1000),
            -32003,
        );
        failed(
            a.handle(&mut store, &create("a1", "empty", " "), 1000),
            -32602,
        );
        failed(
            a.handle(&mut store, &create("a1", "large", &"龙".repeat(1366)), 1000),
            -32602,
        );
        let invalid = String::from_utf8(request.clone())
            .unwrap()
            .replace("task.create\",\"deadline", "task.read\",\"deadline");
        failed(a.handle(&mut store, invalid.as_bytes(), 1000), -32602);
        let invalid = String::from_utf8(request.clone()).unwrap().replace(
            "\"description\":",
            "\"task_id\":\"forged\",\"description\":",
        );
        ...
        failed(
            user.handle(&mut store, &create("desktop", "same", "说明"), 1000),
            -32003,
        );
        ...
        failed(
            a.handle(&mut store, &create("a1", "same", "修改说明"), 1000),
            -32009,
        );
        ...
        assert_eq!(
            yonder_application::list(&mut store, AuthContext::Agent("b2"), None, true, 100)
                .unwrap()
                .tasks
                .len(),
            1
        ...
        let foreign = format!(
            r#"{{"jsonrpc":"2.0","id":"get","method":"task.get","params":{{"agent_id":"b2","capability":"task.read","deadline":2000,"task_id":"{}"}}}}"#,
            first.task_id
        );
        failed(b.handle(&mut store, foreign.as_bytes(), 1000), -32004);
        let counts = |store: &SqliteTaskStore| {
            store.0.query_row("SELECT (SELECT count(*) FROM tasks),(SELECT count(*) FROM events),(SELECT count(*) FROM outbox),(SELECT count(*) FROM task_creations)", [], |r| Ok((r.get::<_,i64>(0)?,r.get::<_,i64>(1)?,r.get::<_,i64>(2)?,r.get::<_,i64>(3)?))).unwrap()
        };
        ...
        failed(
            a.handle(&mut store, &create("a1", "rollback", "说明"), 1000),
            -32603,
        );
        assert_eq!(counts(&store), (3, 3, 3, 2));
        store
    ...
    fn unencrypted_storage_persists_recovers_rolls_back_and_preserves_rejected_files() {
        ...
        assert_eq!(
            transition(&mut store, "a-task", 2, Action::Complete),
        ...
        let count: i64 = store
            .0
            .query_row(
                "SELECT count(*) FROM outbox WHERE task_id='a-task'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        ...
        let second = list(
            &mut store,
            AuthContext::LocalUser("desktop"),
            page.next_after_task_id.as_deref(),
            false,
            1,
        )
        .unwrap();
        ...
        assert_eq!(
            list(&mut store, AuthContext::Agent("a1"), None, true, 100)
                .unwrap()
                .tasks
                .len(),
            1
        ...
        assert_eq!(
            SqlCipherTaskStore::open(&encrypted, &[53; 32])
                .unwrap()
                .get("protected")
                .unwrap()
                .id,
        ...
        for file in [path, invalid, encrypted] {
    ...
    fn rest_reservation_checks_storage_and_blocks_start_without_writing() {
        ...
        assert_eq!(
            reserve_rest(&mut store, None).err(),
            Some(RestDenied::Unavailable)
        );
        let rest = reserve_rest(&mut store, Some(&gate)).unwrap();
        assert_eq!(
            start(&mut store, &gate, "task", 1, &[]).err(),
            Some(StartError::Admission(Denied::PresentationBusy))
        );
        ...
        assert_eq!(
            reserve_rest(&mut store, Some(&gate)).err(),
            Some(RestDenied::Busy)
        );
        ...
        store
            .0
            .execute_batch("ALTER TABLE tasks RENAME TO unavailable_tasks;")
            .unwrap();
        assert_eq!(
            reserve_rest(&mut store, Some(&gate)).err(),
            Some(RestDenied::Unavailable)
        ...
        store
            .0
            .execute_batch("ALTER TABLE unavailable_tasks RENAME TO tasks;")
            .unwrap();
        reserve_rest(&mut store, Some(&gate))
            .unwrap()
            .release_after_wake()
            .unwrap();
        drop(store);
    ...
    fn activity_includes_unreleased_background_work_and_unknown_sources() {
        ...
        assert_eq!(
            activity_state(&mut store, Some(&gate)),
            ActivityState::NoKnownWork
        );
    ...
    fn stopped_task_finishes_before_releasing_and_failure_keeps_permit() {
        ...
        let (other, other_permit) =
            start(&mut store, &gate, "other", 1, &[Resource::Desktop]).unwrap();
        let permit = match permit.finish_after_stop(&mut store, 1, Outcome::Completed) {
            Err(FinishError::Task {
        ...
        assert_eq!(
            gate.try_acquire("blocked", &[Resource::Browser]).err(),
            Some(Denied::ResourceBusy(Resource::Browser))
        );
        ...
        let count: i64 = reader
            .0
            .query_row(
                "SELECT count(*) FROM outbox WHERE task_id='first'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        ...
        let completed = match permit.finish_after_stop(&mut store, 2, Outcome::Completed) {
            Ok(task) => task,
            _ => panic!("终态应成功提交"),
        };
        assert_eq!(reader.get("first").unwrap(), completed);
        assert_eq!(
            (completed.status, completed.sequence),
            (Status::Completed, 3)
        );
        ...
        let count: i64 = reader
            ...
            .unwrap();
        assert_eq!(count, 3);
        gate.try_acquire("next", &[Resource::Browser])
            .unwrap()
            .release_after_stop()
            .unwrap();
        assert_eq!(reader.get("other").unwrap(), other);
        assert_eq!(
            gate.try_acquire("blocked", &[Resource::Desktop]).err(),
            Some(Denied::ResourceBusy(Resource::Desktop))
        );
        let failed = match other_permit.finish_after_stop(&mut store, 2, Outcome::Failed) {
            Ok(task) => task,
            _ => panic!("失败结果应持久化"),
        };
        assert_eq!(reader.get("other").unwrap(), failed);
        assert_eq!(failed.status, Status::Failed);
        gate.try_acquire("next", &[Resource::Desktop])
            .unwrap()
            .release_after_stop()
            .unwrap();
        drop(reader);
    ...
    fn admitted_start_commits_before_return_and_releases_only_failed_attempt() {
        ...
        assert_eq!(
            start(&mut store, &gate, "second", 1, &[file.clone()]).err(),
            Some(StartError::Admission(Denied::ResourceBusy(file.clone())))
        );
        ...
        assert_eq!(
            start(&mut store, &gate, "second", 1, &[Resource::Browser]).err(),
            Some(StartError::Task(Error::StorageUnavailable))
        );
        ...
        let count: i64 = reader
            .0
            .query_row(
                "SELECT count(*) FROM outbox WHERE task_id='second'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        ...
        assert_eq!(
            gate.try_acquire("blocked", &[file.clone()]).err(),
            Some(Denied::ResourceBusy(file))
        );
        gate.try_acquire("probe", &[Resource::Browser])
            .unwrap()
            .release_after_stop()
            .unwrap();
        store
            .0
            .execute_batch("DROP TRIGGER fail_start_outbox;")
            .unwrap();
        assert_eq!(
            start(&mut store, &gate, "second", 0, &[Resource::Browser]).err(),
            Some(StartError::Task(Error::Conflict))
        );
        let (_, second_permit) =
            start(&mut store, &gate, "second", 1, &[Resource::Browser]).unwrap();
        assert_eq!(reader.running(100).unwrap().len(), 2);
        assert_eq!(
            start(&mut store, &gate, "third", 1, &[]).err(),
            Some(StartError::Admission(Denied::Capacity))
        );
        ...
        assert!(matches!(
            start(&mut store, &gate, "second", 3, &[Resource::Browser]).err(),
            Some(StartError::Task(Error::InvalidTransition(_)))
        ));
    ...
    fn task_ownership_applies_to_every_query_and_preserves_legacy_data() {
        ...
        for (id, owner) in [
            ...
            let task =
                yonder_application::create(&mut store, id, AuthContext::Agent(owner)).unwrap();
        ...
        let mut session = yonder_application::gateway::GatewaySession::new(
            AuthContext::Agent("a1"),
            yonder_protocol::Platform::Macos,
        );
        ...
        match session.handle(&mut store, list, 1000) {
            Response::Success {
                ...
                assert_eq!(tasks[0].task_id, "b-middle");
                assert_eq!(tasks[0].owner_agent_id, "a1");
                assert_eq!(next_after_task_id.as_deref(), Some("b-middle"));
            }
            response => panic!("归属筛选失败：{response:?}"),
        }
        let page = yonder_application::list(
            &mut store,
            AuthContext::Agent("a1"),
            Some("b-middle"),
            true,
        ...
        .unwrap();
        assert_eq!(page.tasks[0].id, "c-last");
        assert!(page.next_after_task_id.is_none());
        let b = yonder_application::list(&mut store, AuthContext::Agent("b2"), None, true, 100)
            .unwrap();
        assert_eq!(
            b.tasks.iter().map(|t| t.id.as_str()).collect::<Vec<_>>(),
            vec!["a-first", "d-last"]
        );
        let local = yonder_application::list(
            &mut store,
            AuthContext::LocalUser("desktop"),
            None,
            true,
        ...
        match yonder_application::query::handle(
            &mut store,
            AuthContext::LocalUser("a1"),
            list,
            1000,
        ) {
            Response::Success {
                result: QueryResult::Tasks { tasks, .. },
                ..
            } => assert_eq!(tasks[0].task_id, "a-first"),
            response => panic!("本机用户总览失败：{response:?}"),
        }
        fn failure(response: Response) -> (Option<String>, i32) {
            match response {
                Response::Failure { id, error, .. } => (id, error.code),
                response => panic!("应拒绝：{response:?}"),
            }
        }
        assert_eq!(
            failure(yonder_application::query::handle(
                &mut store,
                AuthContext::Agent("b2"),
                list,
                1000
        ...
        for method in ["task.get", "task.events"] {
            for task_id in ["a-first", "missing", "b-middle"] {
                let extra = if method == "task.events" {
                    ",\"after_sequence\":\"0\",\"limit\":100"
                } else {
                    ""
                };
                let request = format!(
                    r#"{{"jsonrpc":"2.0","id":"r1","method":"{method}","params":{{"agent_id":"a1","capability":"task.read","deadline":2000,"task_id":"{task_id}"{extra}}}}}"#
                );
                let result = session.handle(&mut store, request.as_bytes(), 1000);
                if task_id == "b-middle" {
                    assert!(matches!(result, Response::Success { .. }));
                } else {
                    assert_eq!(failure(result), (Some("r1".into()), -32004));
                }
                assert_eq!(
                    failure(yonder_application::query::handle(
                        &mut store,
    ...
    fn task_list_pages_committed_states_without_duplicates_or_writes() {
        ...
        for (id, action) in [
            ("t-000", Action::Complete),
            ("t-001", Action::Fail),
            ("t-002", Action::Cancel),
            ("t-003", Action::Pause),
            ("t-004", Action::WaitForUser),
            ("t-005", Action::Interrupt),
        ] {
        ...
        let running =
            list_running(&mut reader, AuthContext::LocalUser("local-test"), None, 1).unwrap();
        assert_eq!(
            running
                .tasks
                .iter()
        ...
        .unwrap();
        assert_eq!(
            last.tasks.iter().map(|t| t.id.as_str()).collect::<Vec<_>>(),
            vec!["t-103", "t-104"]
        );
        ...
        match yonder_application::query::handle(
            ...
            Response::Success {
                ...
                assert_eq!(id, "list-1");
                assert_eq!(tasks[0].task_id, "a-new");
        ...
        assert!(
            matches!(yonder_application::query::handle(&mut reader, AuthContext::LocalUser("local-test"), filtered, 1000), Response::Success { result: QueryResult::Tasks { tasks, next_after_task_id: None }, .. } if tasks.len() == 1 && tasks[0].task_id == "t-006")
        );
        ...
        assert!(
            matches!(old.handle(&mut reader, agent_filtered, 1000), Response::Failure { error, .. } if error.code == -32010)
        );
        let mut current = GatewaySession::new(AuthContext::Agent("a1"), Platform::Macos);
        assert!(
            matches!(current.handle(&mut reader, hello(16).as_bytes(), 1000), Response::Success { result: QueryResult::Hello { protocol_version, .. }, .. } if protocol_version.minor == 16)
        );
        assert!(
            matches!(current.handle(&mut reader, agent_filtered, 1000), Response::Success { result: QueryResult::Tasks { tasks, .. }, .. } if tasks.len() == 1 && tasks[0].task_id == "t-006")
        );
        let outbox_after: i64 = writer
            .0
            .query_row("SELECT count(*) FROM outbox", [], |r| r.get(0))
            .unwrap();
    ...
    fn restart_recovery_is_bounded_repeatable_and_preserves_atomic_events() {
        ...
        for id in ["b", "c"] {
            ...
            let count: i64 = store
                .0
                .query_row("SELECT count(*) FROM outbox WHERE task_id=?1", [id], |r| {
                    r.get(0)
                })
                .unwrap();
            assert_eq!(count, 2);
        ...
        for id in ["a", "b", "c"] {
            ...
            assert_eq!(
                store.events(id, 2, 100).unwrap(),
                vec![Status::Running.transition(2, Action::Interrupt).unwrap()]
            );
            let count: i64 = store
                .0
                .query_row("SELECT count(*) FROM outbox WHERE task_id=?1", [id], |r| {
        ...
        for snapshot in unchanged {
            ...
            assert_eq!(
                store.events(&snapshot.id, snapshot.sequence, 100).unwrap(),
                vec![]
            );
        }
        drop(store);
    ...
    fn agent_step_declaration_is_versioned_idempotent_and_auditable() {
        ...
        let task = match session.handle(&mut store, create.as_bytes(), 1000) {
            Response::Success {
                result: QueryResult::Snapshot { task },
                ..
            } => task,
        ...
        let declare = |label: &str, expected: &str| {
            format!(
                r#"{{"jsonrpc":"2.0","id":"d","method":"task.step.declare","params":{{"agent_id":"a1","capability":"task.step.declare","deadline":2000,"task_id":"{}","expected_sequence":"{expected}","step_id":"step-1","label":"{label}"}}}}"#,
                task.task_id
            )
        };
        let first = match session.handle(&mut store, declare("打开文档", "1").as_bytes(), 1000)
        {
            Response::Success {
                result:
                    QueryResult::Step {
        ...
        assert_eq!(
            (
                first.0.status,
                first.0.sequence.as_str(),
                first.1.accepted_sequence.as_str()
            ),
            (TaskStatus::Created, "2", "2")
        );
        assert!(
            matches!(session.handle(&mut store, declare("打开文档", "1").as_bytes(), 1000), Response::Success { result:QueryResult::Step { task, step:Some(_) }, .. } if task.sequence == "2")
        );
        assert!(
            matches!(session.handle(&mut store, declare("其他步骤", "2").as_bytes(), 1000), Response::Failure { error, .. } if error.code == -32013)
        );
        let get = format!(
            r#"{{"jsonrpc":"2.0","id":"g","method":"task.step.get","params":{{"agent_id":"a1","capability":"task.read","deadline":2000,"task_id":"{}"}}}}"#,
            task.task_id
        );
        assert!(
            matches!(session.handle(&mut store, get.as_bytes(), 1000), Response::Success { result:QueryResult::Step { step:Some(step), .. }, .. } if step.label == "打开文档")
        );
        let events = format!(
            r#"{{"jsonrpc":"2.0","id":"e","method":"task.events","params":{{"agent_id":"a1","capability":"task.read","deadline":2000,"task_id":"{}","after_sequence":"0","limit":100}}}}"#,
            task.task_id
        );
        ...
        .unwrap();
        assert!(
            matches!(session.handle(&mut store, declare("打开文档", "1").as_bytes(), 1000), Response::Success { result:QueryResult::Step { task, .. }, .. } if task.status == TaskStatus::Cancelled)
        );
        assert_eq!(store.events(&task.task_id, 0, 100).unwrap().len(), 3);
        assert_eq!(
            store
                .0
                .query_row("SELECT count(*) FROM task_steps", [], |r| r
                    .get::<_, i64>(0))
                .unwrap(),
            1
        );
        ...
        let second_create = create
            .replace("step-task", "atomic-task")
            .replace("步骤测试", "原子回滚");
        let second = match session.handle(&mut store, second_create.as_bytes(), 1000) {
            Response::Success {
                result: QueryResult::Snapshot { task },
                ..
            } => task,
        ...
        assert!(
            matches!(session.handle(&mut store, second_declare.as_bytes(), 1000), Response::Failure { error, .. } if error.code == -32603)
        );
        assert_eq!(store.get(&second.task_id).unwrap().sequence, 1);
        assert_eq!(store.get_with_step(&second.task_id).unwrap().1, None);
        store
            .0
            .execute_batch("DROP TRIGGER fail_step_outbox;")
            .unwrap();
    }
    ...
    fn execution_attempt_preparation_is_atomic_idempotent_and_step_bound() {
        ...
        let gate = Admission::new(1).unwrap();
        let (running, accepted, permit) = start_attempt(
            &mut store,
            &gate,
            &requested,
            task.sequence,
        ...
        .unwrap();
        assert_eq!(
            (running.status, running.sequence, accepted.accepted_sequence),
            (Status::Running, 3, 3)
        );
        assert_eq!(store.get_attempt(&task.id).unwrap(), Some(accepted.clone()));
        assert_eq!(
            prepare_attempt(&mut store, &requested, task.sequence).unwrap(),
            (running.clone(), accepted.clone())
        );
        let different = ExecutionAttempt {
            attempt_id: "attempt-2".into(),
            ..requested.clone()
        };
        assert_eq!(
            prepare_attempt(&mut store, &different, task.sequence),
            Err(Error::Conflict)
        );
        assert_eq!(store.events(&task.id, 0, 100).unwrap().len(), 3);

        let (observed, result) = record_dispatch_outcome(
            &mut store,
            &task.id,
            &accepted.attempt_id,
            DispatchOutcome::Known {
        ...
        .unwrap();
        assert_eq!(
            (observed.sequence, result.result_sequence, result.conclusion),
            (
                4,
                4,
                AttemptConclusion::Observed {
        ...
        assert_eq!(
            store.events_with_steps(&task.id, 3, 1).unwrap()[0].attempt_result,
            Some(result)
        );
        for (minor, visible) in [(4, false), (5, true)] {
            ...
            session.handle(&mut store, hello.as_bytes(), 1000);
            let events = format!(
                r#"{{"jsonrpc":"2.0","id":"e","method":"task.events","params":{{"agent_id":"a1","capability":"task.read","deadline":2000,"task_id":"{}","after_sequence":"3","limit":10}}}}"#,
                task.id
            );
            let encoded = String::from_utf8(
                yonder_protocol::encode(&session.handle(&mut store, events.as_bytes(), 1000))
                    .unwrap(),
            )
            .unwrap();
            assert_eq!(encoded.contains("attempt_result"), visible);
        }
        let stale = ExecutionAttempt {
            worker_instance_id: "old-worker".into(),
            ..accepted.clone()
        };
        let (controlled, control) = request_control(
            &mut store,
            AuthContext::LocalUser("desktop"),
            &task.id,
            observed.sequence,
        ...
        .unwrap();
        assert_eq!(
            (controlled.status, controlled.sequence, control.phase),
            (Status::Running, 5, ControlPhase::Pending)
        );
        assert_eq!(
            request_control(
                &mut store,
            ...
            .unwrap(),
            (controlled.clone(), control.clone())
        );
        assert_eq!(
            store.stop_attempt(
                &stale,
        ...
        let (paused, stop) = match permit.stop_at_boundary(
            &mut store,
            &accepted.attempt_id,
            ControlKind::Takeover,
        ) {
        ...
        assert_eq!(
            ...
            (Status::Paused, 6, ControlKind::Takeover, 6)
        );
        assert_eq!(
            store.get_attempt(&task.id).unwrap().unwrap().phase,
        ...
        impl WorkFocusPort for Focus {
            fn capture(
                ...
                Err(FocusFailure::ReferenceUnavailable)
            }
        ...
        assert_eq!(
            focus_after_takeover(&mut store, &mut Focus, &reference),
            Ok(FocusOutcome::Focused)
        );
        assert_eq!(
            focus_after_takeover(
                &mut store,
                &mut Focus,
                &WorkRef {
                    worker_instance_id: "old-worker".into(),
        ...
        let no_step = store
            ...
            .unwrap();
        let missing = ExecutionAttempt {
            task_id: no_step.id.clone(),
            ..requested.clone()
        };
        assert_eq!(
            prepare_attempt(&mut store, &missing, no_step.sequence),
            Err(Error::StopRequired)
        );
        let rollback = store
            .register(
        ...
        let failed = ExecutionAttempt {
            task_id: rollback.id.clone(),
            attempt_id: "attempt-rollback".into(),
            ..requested
        };
        assert_eq!(
            prepare_attempt(&mut store, &failed, rollback.sequence),
            Err(Error::StorageUnavailable)
        );
        assert_eq!(
            (
                store.get(&rollback.id).unwrap().status,
                store.get_attempt(&rollback.id).unwrap()
            ),
            (Status::Created, None)
        );
        store
            .0
            .execute_batch("DROP TRIGGER fail_attempt_outbox;")
            .unwrap();
        let gate = Admission::new(1).unwrap();
        let (_, accepted, unknown_permit) = start_attempt(
            &mut store,
            &gate,
            &failed,
            rollback.sequence,
        ...
        assert_eq!(
            (
                store.get(&rollback.id).unwrap().sequence,
                store.get_attempt(&rollback.id).unwrap().unwrap().phase
            ),
            (3, AttemptPhase::Prepared)
        );
        store
            .0
            .execute_batch("DROP TRIGGER fail_result_outbox;")
            .unwrap();
        let (unknown_task, unknown) = record_dispatch_outcome(
            &mut store,
            &rollback.id,
            &accepted.attempt_id,
            DispatchOutcome::Unknown(UnknownReason::TimedOut),
        ...
        assert_eq!(
            yonder_application::advance_after_observe(
                &mut store,
                &rollback.id,
                &accepted.attempt_id
            ),
        ...
        let (stop_task, _) = store
            ...
            .unwrap();
        let stop_attempt = ExecutionAttempt {
            task_id: stop_task.id.clone(),
            step_id: "stop-step".into(),
            attempt_id: "stop-attempt".into(),
            worker_instance_id: "stop-worker".into(),
            host_session_id: "stop-host".into(),
            phase: AttemptPhase::Prepared,
            accepted_sequence: 0,
        };
        let stop_gate = Admission::new(1).unwrap();
        let (_, stop_attempt, stop_permit) = start_attempt(
            &mut store,
            &stop_gate,
            &stop_attempt,
            stop_task.sequence,
        ...
        impl ComputerUsePort for MustNotDispatch {
            fn dispatch(
                &self,
                _: &ExecutionAttempt,
                _: &WorkTarget,
        ...
        assert_eq!(
            dispatch_prepared(
                ...
                &ComputerAction {
                    ...
                    arguments_json: r#"{"text":"x"}"#.into()
                }
            ),
            Err(Error::StopRequired)
        );
        let (stop_running, _) = record_dispatch_outcome(
            &mut store,
            &stop_task.id,
            &stop_attempt.attempt_id,
            DispatchOutcome::Known {
        ...
        let stop_permit = match stop_permit.stop_at_boundary(
            &mut store,
            &stop_attempt.attempt_id,
            ControlKind::Cancel,
        ) {
            Err(BoundaryStopError::Task {
                error: Error::StorageUnavailable,
                permit,
            }) => permit,
            _ => panic!("停止事务应回滚"),
        ...
        assert_eq!(
            (
                store.get(&stop_task.id).unwrap().sequence,
                store.get_attempt(&stop_task.id).unwrap().unwrap().phase
            ),
            (stop_running.sequence, AttemptPhase::Observed)
        );
        assert!(stop_gate.has_occupancy().unwrap());
        store
            .0
            .execute_batch("DROP TRIGGER fail_boundary_outbox;")
            .unwrap();
        let (cancelled, _) = match stop_permit.stop_at_boundary(
            &mut store,
            &stop_attempt.attempt_id,
            ControlKind::Cancel,
        ) {
    ...
    fn takeover_at_safe_boundary_persists_focus_without_recording() {
        ...
        let requested = ExecutionAttempt {
            task_id: task.id.clone(),
            step_id: "edit".into(),
            attempt_id: "attempt-focus".into(),
            worker_instance_id: "worker-focus".into(),
            host_session_id: "host-focus".into(),
            phase: AttemptPhase::Prepared,
            accepted_sequence: 0,
        };
        let gate = Admission::new(1).unwrap();
        ...
        let (boundary, _) =
            yonder_application::advance_after_observe(&mut store, &task.id, &attempt.attempt_id)
                .unwrap();
        let (paused, control) = yonder_application::request_control(
            &mut store,
            AuthContext::LocalUser("desktop"),
            &task.id,
            boundary.sequence,
        ...
        impl WorkFocusPort for Focus {
            ...
            ) -> Result<WorkRef, yonder_application::work_focus::FocusFailure> {
                unreachable!()
            }
            fn focus(&mut self, _: &WorkRef) -> FocusOutcome {
                FocusOutcome::Focused
    ...
    fn start_execution_routes_created_and_running_tasks() {
        ...
        let gate = Admission::new(1).unwrap();
        let (running, first) = start_execution(
            &mut store,
            &gate,
            &task,
        ...
        .unwrap();
        assert_eq!(
            (running.status, running.sequence, first.accepted_sequence),
            (Status::Running, task.sequence + 1, task.sequence + 1)
        );
        assert_eq!(gate.holds_resource(&task.id, Resource::Desktop), Ok(true));
        let (task, _) = record_dispatch_outcome(
            &mut store,
            &task.id,
            &first.attempt_id,
            DispatchOutcome::Known {
        ...
        let (task, _) = store
            .declare_step("a1", &task.id, task.sequence, "step-2", "第二步")
            .unwrap();
        let (next, second) = start_execution(
            &mut store,
            &gate,
            &task,
        ...
        .unwrap();
        assert_eq!(
            (next.status, next.sequence, second.accepted_sequence),
            (Status::Running, task.sequence + 1, task.sequence + 1)
        );
        assert_eq!(gate.holds_resource(&task.id, Resource::Desktop), Ok(true));
    }
    ...
    fn browser_reference_commits_with_observe_and_survives_later_unknown() {
        ...
        let task = store
            ...
            .unwrap();
        let (task, _) = store
            .declare_step("a1", &task.id, task.sequence, "create", "创建空间")
            .unwrap();
        let attempt = |step: &str, id: &str| ExecutionAttempt {
            task_id: task.id.clone(),
            step_id: step.into(),
        ...
        let gate = Admission::new(1).unwrap();
        let (_, first, _permit) = start_attempt(
            &mut store,
            &gate,
        ...
        let get = format!(
            r#"{{"jsonrpc":"2.0","id":"g","method":"task.browser.get","params":{{"agent_id":"a1","capability":"task.read","deadline":2000,"task_id":"{}"}}}}"#,
            task.id
        );
        let mut session = GatewaySession::new(AuthContext::Agent("a1"), Platform::Macos);
        assert!(
            matches!(session.handle(&mut store,hello,1000),Response::Success{result:QueryResult::Hello{protocol_version,..},..} if protocol_version.minor==15)
        );
        assert!(matches!(
            session.handle(&mut store, get.as_bytes(), 1000),
            Response::Success {
                result: QueryResult::BrowserState {
                    reference: None,
                    ..
                },
                ..
        ...
        assert!(
            matches!(other.handle(&mut store,other_get.as_bytes(),1000),Response::Failure{error,..} if error.code==-32004)
        );
        ...
        let outcome = BrowserOutcome::Observed(BrowserTaskRef {
            external_task_ref: "ego:41".into(),
            ownership: "agent".into(),
            managed_pages: 1,
        });
        assert_eq!(
            record_outcome(&mut store, &task.id, &first.attempt_id, &outcome),
            Err(Error::StorageUnavailable)
        );
        assert_eq!(store.get_browser_reference(&task.id).unwrap(), None);
        store
            .0
            .execute_batch("DROP TRIGGER fail_browser_outbox;")
            .unwrap();
        let (observed, _) =
            record_outcome(&mut store, &task.id, &first.attempt_id, &outcome).unwrap();
        let reference = store.get_browser_reference(&task.id).unwrap().unwrap();
        assert_eq!(
            (
                reference.external_task_ref.as_str(),
                reference.ownership.as_str(),
                reference.managed_pages,
                reference.finished,
                reference.updated_sequence
            ),
            ("ego:41", "agent", 1, false, observed.sequence)
        );
        assert!(
            matches!(session.handle(&mut store,get.as_bytes(),1000),Response::Success{result:QueryResult::BrowserState{reference:Some(value),..},..} if value.external_task_ref=="ego:41"&&value.updated_sequence==observed.sequence.to_string())
        );
        let (advanced, _) = advance_after_observe(&mut store, &task.id, &first.attempt_id).unwrap();
        let (declared, _) = store
            .declare_step("a1", &task.id, advanced.sequence, "handoff", "交给用户")
            .unwrap();
        struct Handoff;
        impl BrowserUsePort for Handoff {
            fn dispatch(&self, _: &ExecutionAttempt, action: &BrowserAction) -> BrowserOutcome {
                match action {
                    BrowserAction::HandOff { external_task_ref } => {
                        BrowserOutcome::Observed(BrowserTaskRef {
                            external_task_ref: external_task_ref.clone(),
                            ownership: "agentDelegatedToUser".into(),
                            managed_pages: 1,
                        })
                    }
        ...
        let (handed, reference) = execute_agent_action(
            &mut store,
            &gate,
            &Handoff,
            AuthContext::LocalUser("desktop"),
        ...
        let (advanced, _) =
            advance_after_observe(&mut store, &task.id, &current.attempt_id).unwrap();
        let (declared, _) = store
            .declare_step("a1", &task.id, advanced.sequence, "finish", "结束空间")
            .unwrap();
        let (_, second) = prepare_next_attempt(
            &mut store,
            &attempt("finish", "attempt-2"),
            declared.sequence,
        )
        .unwrap();
        let (finished_task, _) = record_outcome(
            &mut store,
            &task.id,
            &second.attempt_id,
            &BrowserOutcome::Finished {
                external_task_ref: "ego:41".into(),
            },
        )
        .unwrap();
        ...
        assert_eq!(finished.updated_sequence, finished_task.sequence);
        let (advanced, _) =
            advance_after_observe(&mut store, &task.id, &second.attempt_id).unwrap();
        let (declared, _) = store
            .declare_step("a1", &task.id, advanced.sequence, "wait", "等待")
            .unwrap();
        let (_, third) =
            prepare_next_attempt(&mut store, &attempt("wait", "attempt-3"), declared.sequence)
                .unwrap();
        record_dispatch_outcome(
            &mut store,
            &task.id,
        ...
        .unwrap();
        assert_eq!(
            store.get_browser_reference(&task.id).unwrap().unwrap(),
            finished
        );
    ...
    fn agent_cua_supervisor_completes_and_user_input_interrupts() {
        ...
        let task = store
            ...
            .unwrap();
        let (task, _) = store
            .declare_step("a1", &task.id, task.sequence, "type", "输入文本")
            .unwrap();
        let (observed, result, _) = execute_agent_action(
            &mut store,
            &gate,
            &Port(DispatchOutcome::Known {
                action_succeeded: true,
                observation: None,
            }),
            &Target,
        ...
        assert!(gate.holds_resource(&task.id, Resource::Desktop).unwrap());
        ...
        .unwrap();
        assert_eq!(
            (completed.status, gate.has_occupancy()),
        ...
        let failed = store
            ...
            .unwrap();
        let (failed, _) = store
            .declare_step("a1", &failed.id, failed.sequence, "click", "点击按钮")
            .unwrap();
        let (_, result, _) = execute_agent_action(
            &mut store,
            &gate,
            &Port(DispatchOutcome::Known {
                action_succeeded: false,
        ...
        let attempt = store.get_attempt(&failed.id).unwrap().unwrap();
        let (boundary, _) =
            advance_after_observe(&mut store, &failed.id, &attempt.attempt_id).unwrap();
        assert_eq!(
            complete_agent_task(
                &mut store,
                &gate,
        ...
        let _ = session
            .handle_encoded_with_runtimes(
                ...
                &gate,
                None,
                Some(&Port(DispatchOutcome::Known {
                    action_succeeded: false,
                    observation: None,
                })),
                Some(&Target),
            ...
            .unwrap();
        let request = format!(
            r#"{{"jsonrpc":"2.0","id":"fail","method":"task.fail","params":{{"agent_id":"a1","capability":"task.fail","deadline":2000,"task_id":"{}","expected_sequence":"{}"}}}}"#,
            failed.id, boundary.sequence
        );
        let (response, _) = session
            ...
            .unwrap();
        assert!(
            matches!(yonder_protocol::decode_response(&response),Ok(Response::Success{result:QueryResult::Snapshot{task},..}) if task.status==TaskStatus::Failed)
        );
        ...
        let (interrupted, _) = store
            ...
            .unwrap();
        let (task, result, _) = execute_agent_action(
            &mut store,
            &gate,
            &Port(DispatchOutcome::Unknown(UnknownReason::UserInput)),
            &Target,
            AuthContext::Agent("a1"),
            &interrupted.id,
            interrupted.sequence,
        ...
        .unwrap();
        assert_eq!(
            (task.status, result.conclusion, gate.has_occupancy()),
            (
                Status::Interrupted,
                AttemptConclusion::Unknown {
                    reason: UnknownReason::UserInput
                },
                Ok(false)
            )
        );
    ...
    fn observed_boundary_allows_next_step_without_releasing_task_permit() {
        ...
        assert_eq!(
            advance_after_observe(&mut store, &task.id, &first.attempt_id),
            Err(Error::StorageUnavailable)
        );
        assert_eq!(
            (
                store.get(&task.id).unwrap().sequence,
                store.get_attempt(&task.id).unwrap().unwrap().phase
            ),
            (observed.sequence, AttemptPhase::Observed)
        );
        store
            .0
            .execute_batch("DROP TRIGGER fail_advance_outbox;")
            .unwrap();
        ...
        .unwrap();
        let (_observed, _) = record_dispatch_outcome(
            &mut store,
            &task.id,
            &second.attempt_id,
            DispatchOutcome::Known {
        ...
        assert_eq!(
            store.events(&task.id, 0, 100).unwrap().len(),
            advanced.sequence as usize
        );
        let completed = permit
        ...
        let blocked = store
            ...
            .unwrap();
        let (blocked, _) = store
            .declare_step("a1", &blocked.id, blocked.sequence, "step", "步骤")
            .unwrap();
        let request = ExecutionAttempt {
        ...
        let gate = Admission::new(1).unwrap();
        let (_, accepted, _permit) = start_attempt(
            &mut store,
            &gate,
            &request,
            blocked.sequence,
        ...
        .unwrap();
        yonder_application::request_control(
            &mut store,
            AuthContext::LocalUser("desktop"),
            &blocked.id,
            observed.sequence,
        ...
        .unwrap();
        assert_eq!(
            advance_after_observe(&mut store, &blocked.id, &accepted.attempt_id),
            Err(Error::StopRequired)
        );
    }
    ...
    fn agent_waits_only_at_observed_boundary_and_persists_reason() {
        ...
        let task = store
            ...
            .unwrap();
        let (task, _) = store
            .declare_step("a1", &task.id, task.sequence, "step", "准备询问")
            .unwrap();
        ...
        let gate = Admission::new(1).unwrap();
        let (_, attempt, permit) = start_attempt(
            &mut store,
            &gate,
            &attempt,
            task.sequence,
        ...
        let request = format!(
            r#"{{"jsonrpc":"2.0","id":"w","method":"task.wait_for_user","params":{{"agent_id":"a1","capability":"task.wait-for-user","deadline":2000,"task_id":"{}","expected_sequence":"{}","reason":"请确认发送内容"}}}}"#,
            task.id, boundary.sequence
        );
        let mut old = GatewaySession::new(AuthContext::Agent("a1"), Platform::Macos);
        ...
        let (bytes, _) = old
            ...
            .unwrap();
        assert!(
            matches!(yonder_protocol::decode_response(&bytes).unwrap(),Response::Failure{error,..} if error.code==-32010)
        );
        assert_eq!(
            (store.get(&task.id).unwrap().status, gate.has_occupancy()),
        ...
        let (bytes, _) = current
            ...
            .unwrap();
        assert!(
            matches!(yonder_protocol::decode_response(&bytes).unwrap(),Response::Success{result:QueryResult::Snapshot{task},..} if task.status==TaskStatus::WaitingForUser)
        );
        assert_eq!(gate.has_occupancy(), Ok(false));
        let event = store
            ...
            .unwrap();
        assert_eq!(
            (event.transition.next, event.wait_reason.as_deref()),
            (Status::WaitingForUser, Some("请确认发送内容"))
        );
        let events = format!(
            r#"{{"jsonrpc":"2.0","id":"e","method":"task.events","params":{{"agent_id":"a1","capability":"task.read","deadline":2000,"task_id":"{}","after_sequence":"{}","limit":1}}}}"#,
            task.id, boundary.sequence
        );
        assert!(
            matches!(current.handle(&mut store,events.as_bytes(),1000),Response::Success{result:QueryResult::Events{events,..},..} if events[0].wait_reason.as_deref()==Some("请确认发送内容"))
        );
        assert!(
    ...
    fn encrypted_transactions_reject_stale_writes_and_rollback_outbox_failure() {
        ...
        let key = [17; 32]; // 仅合成测试库使用。
        {
            ...
            let stale_change = stale
                .status
                .transition(stale.sequence, Action::Start)
                .unwrap();
            assert_eq!(
                second.commit("task-1", 1, stale_change),
                Err(Error::Conflict)
            );
            ...
            assert_eq!(
                transition(&mut first, "task-1", 2, Action::Complete),
                Err(Error::StorageUnavailable)
            );
