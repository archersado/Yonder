...
pub enum AttemptConclusion {
    Observed { action_succeeded: bool },
    Unknown { reason: computer_use::UnknownReason },
}
...
pub struct AttemptResultRecord {
    ...
    pub worker_instance_id: String,
    pub host_session_id: String,
    pub conclusion: AttemptConclusion,
...
pub struct ControlRequestRecord {
    ...
    pub focus_failure: Option<work_focus::FocusFailure>,
}
...
impl AuthContext<'_> {
    pub fn agent_id(&self) -> &str {
        match self {
            Self::Agent(id) | Self::LocalUser(id) => id,
        }
    }
    pub fn owner_filter(&self) -> Option<&str> {
        match self {
            Self::Agent(id) => Some(id),
            Self::LocalUser(_) => None,
        }
    }
    pub fn can_read(&self, task: &Task) -> bool {
        self.owner_filter()
            .is_none_or(|id| id == task.owner_agent_id)
    }
}
...
pub trait TaskStore {
    ...
    fn get_with_step(&mut self, id: &str) -> Result<(Task, Option<StepDeclaration>), Error> {
        self.get(id).map(|task| (task, None))
    }
    fn events_with_steps(
        ...
        self.events(id, after, limit).map(|events| {
            events
                .into_iter()
    ...
    fn get_attempt_result(&mut self, _: &str) -> Result<Option<AttemptResultRecord>, Error> {
        Err(Error::StorageUnavailable)
    }
    ...
    fn record_browser_result(
        ...
        _: &ExecutionAttempt,
        _: u64,
        _: &browser_use::BrowserReferenceRecord,
    ) -> Result<(Task, AttemptResultRecord), Error> {
        Err(Error::StorageUnavailable)
    }
    fn get_browser_reference(
        &mut self,
        _: &str,
    ) -> Result<Option<browser_use::BrowserReferenceRecord>, Error> {
        Err(Error::StorageUnavailable)
    }
    fn request_control(
        &mut self,
    ...
    fn get_control(&mut self, _: &str) -> Result<Option<ControlRequestRecord>, Error> {
        Err(Error::StorageUnavailable)
    }
    fn begin_focus(&mut self, _: &str, _: &str) -> Result<(Task, ControlRequestRecord), Error> {
        Err(Error::StorageUnavailable)
    }
    fn finish_focus(
        &mut self,
        _: &str,
        _: &str,
        _: Option<work_focus::FocusFailure>,
    ) -> Result<(Task, ControlRequestRecord), Error> {
        Err(Error::StorageUnavailable)
    }
    fn stop_attempt(
    ...
    fn create(&mut self, id: &str, owner_agent_id: &str, source: TaskSource)
    -> Result<Task, Error>;
    ...
    fn commit(&mut self, id: &str, expected_sequence: u64, change: Transition)
    -> Result<(), Error>;
}
...
pub fn create(store: &mut impl TaskStore, id: &str, auth: AuthContext<'_>) -> Result<Task, Error> {
    ...
    store.create(id, auth.agent_id(), TaskSource::LocalAgent)
}
...
pub fn register(
    ...
    if description.trim().is_empty()
        ...
        || name.is_some_and(|name| !valid_task_name(name))
    {
        return Err(Error::InvalidInput);
    }
...
pub fn record_observation(
    ...
    store.record_observation(task_id, attempt_id, expected, result, summary)
}

pub fn declare_next_intent(
    store: &mut impl TaskStore,
    auth: AuthContext<'_>,
    task_id: &str,
    expected: u64,
    ...
    store.declare_next_intent(auth.owner_filter(), task_id, expected, intent)
}
...
pub fn declare_step(
    ...
    validate_id(task_id)?;
    validate_id(step_id)?;
    if expected == 0 || expected > i64::MAX as u64 || !valid_step_label(label) {
        return Err(Error::InvalidInput);
    }
    store.declare_step(auth.agent_id(), task_id, expected, step_id, label)
}

pub fn get_with_step(
    store: &mut impl TaskStore,
    auth: AuthContext<'_>,
    task_id: &str,
) -> Result<(Task, Option<StepDeclaration>), Error> {
    validate_id(auth.agent_id())?;
    validate_id(task_id)?;
    let result = store.get_with_step(task_id)?;
    if !auth.can_read(&result.0) {
...
pub fn list(
    ...
    let next_after_task_id = if more {
...
pub fn cancel_pending(
    ...
    if task.status == Status::Cancelled
        && (expected == task.sequence || expected.checked_add(1) == Some(task.sequence))
    {
        return Ok(task);
    }
...
pub fn transition(
    ...
    let change = task
        .status
        .transition(task.sequence, action)
...
pub fn wait_for_user(
    ...
    if store
        .get_control(id)?
        .is_some_and(|control| control.phase == ControlPhase::Pending)
        || !store
            .get_attempt(id)?
            .is_some_and(|attempt| attempt.phase == AttemptPhase::Stopped)
    {
        return Err(Error::StopRequired);
    }
...
pub fn stop_at_boundary(
    store: &mut impl TaskStore,
    task_id: &str,
    attempt_id: &str,
    kind: ControlKind,
) -> Result<(Task, StopRecord), Error> {
    if !valid_id(task_id) || !valid_id(attempt_id) {
        return Err(Error::InvalidInput);
    }
    ...
    let control = store
        .get_control(task_id)?
        .filter(|control| control.attempt_id == attempt_id && control.kind == kind)
...
pub fn request_control(
    ...
    if attempt.phase == AttemptPhase::Unknown {
        return Err(Error::StopRequired);
    }
...
mod tests {
    ...
    impl TaskStore for Store {
        ...
        fn running(&mut self, limit: usize) -> Result<Vec<Task>, Error> {
            Ok(std::iter::once(self.task.clone())
        ...
        fn commit(&mut self, _: &str, expected: u64, change: Transition) -> Result<(), Error> {
            ...
            if expected != self.task.sequence {
                return Err(Error::Conflict);
            }
