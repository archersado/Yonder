...
use yonder_protocol::{AttemptResult as ProtocolAttemptResult, AttemptResultPhase as ProtocolAttemptPhase, AttemptUnknownReason as ProtocolUnknownReason, BrowserReference, ControlKind as ProtocolControlKind, ControlPhase as ProtocolControlPhase, ControlRecord as ProtocolControl, FocusPhase as ProtocolFocusPhase, FocusFailure as ProtocolFocusFailure, QueryResult, Request, Response, RpcError, StepDeclaration as ProtocolStep, TaskEvent, TaskSnapshot, TaskStatus, Version};
...
pub fn handle_encoded(store: &mut impl TaskStore, auth: AuthContext<'_>, bytes: &[u8], now_ms: u64) -> Result<Vec<u8>, Error> {
    yonder_protocol::encode(&handle(store, auth, bytes, now_ms)).map_err(|_| Error::StorageUnavailable)
}

/// 可信同版本组合根读取当前事件形状；外部 Agent 仍由 Gateway 协商版本。
pub fn handle_encoded_current(store: &mut impl TaskStore, auth: AuthContext<'_>, bytes: &[u8], now_ms: u64) -> Result<Vec<u8>, Error> {
    let response = match yonder_protocol::decode(bytes) {
        Ok(request) if matches!(request, Request::Cancel { .. }) && matches!(auth, AuthContext::Agent(_)) => Response::Failure {
            jsonrpc: Version::V2, id: Some(request.request_id().into()), error: RpcError::new(-32002, "取消须通过Gateway会话")
        },
        Ok(request) => handle_request_versioned(store, auth, request, now_ms, true, true, true),
        Err(error) => Response::Failure { jsonrpc: Version::V2, id: None, error },
    };
    yonder_protocol::encode(&response).map_err(|_| Error::StorageUnavailable)
}

pub fn handle(store: &mut impl TaskStore, auth: AuthContext<'_>, bytes: &[u8], now_ms: u64) -> Response {
    ...
    if matches!(request, Request::Cancel { .. }) && matches!(auth, AuthContext::Agent(_)) {
        return Response::Failure { jsonrpc: Version::V2, id: Some(request.request_id().into()), error: RpcError::new(-32002, "取消须通过Gateway会话") };
    }
    handle_request(store, auth, request, now_ms)
}

pub(crate) fn validate(request: &Request, auth: AuthContext<'_>, now_ms: u64) -> Result<(), RpcError> {
...
pub(crate) fn handle_request(store: &mut impl TaskStore, auth: AuthContext<'_>, request: Request, now_ms: u64) -> Response {
    handle_request_versioned(store, auth, request, now_ms, false, false, false)
}

pub(crate) fn handle_request_versioned(store: &mut impl TaskStore, auth: AuthContext<'_>, request: Request, now_ms: u64, include_steps: bool, include_attempt_results: bool, include_wait_reason: bool) -> Response {
    ...
    let result = validate(&request, auth, now_ms).and_then(|()| {
        ...
        Request::Cancel { params, .. } => {
            ...
            Ok(QueryResult::Snapshot { task: TaskSnapshot { task_id: task.id, owner_agent_id: task.owner_agent_id, name: task.name, status: status(task.status), sequence: task.sequence.to_string() } })
        },
        ...
        Request::List { params, .. } => {
            ...
            Ok(QueryResult::Tasks {
                tasks: page.tasks.into_iter().map(|task| TaskSnapshot { task_id: task.id, owner_agent_id: task.owner_agent_id, name: task.name, status: status(task.status), sequence: task.sequence.to_string() }).collect(),
        ...
        Request::StepGet { params, .. } => {
            let (task, step) = crate::get_with_step(store, auth, &params.task_id).map_err(error)?;
            Ok(QueryResult::Step { task: TaskSnapshot { task_id: task.id, owner_agent_id: task.owner_agent_id, name: task.name, status: status(task.status), sequence: task.sequence.to_string() }, step: step.map(|step| ProtocolStep { step_id: step.step_id, label: step.label, accepted_sequence: step.accepted_sequence.to_string() }) })
        },
        Request::BrowserGet { params, .. } => {
            ...
            Ok(QueryResult::BrowserState { task: TaskSnapshot { task_id: task.id, owner_agent_id: task.owner_agent_id, name: task.name, status: status(task.status), sequence: task.sequence.to_string() }, reference })
        }
    }});
    match result {
