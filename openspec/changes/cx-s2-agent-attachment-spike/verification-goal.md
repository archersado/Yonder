...
| 非法输入 | 第二个/重复开始、超限、乱序、哈希错误 | 全部稳定拒绝并清理所属会话 | 待复核 |
| 会话隔离 | 第二会话引用第一会话附件 | 拒绝且不读取、不删除原会话附件 | 待复核 |
| 生命周期 | 输入拒绝/unknown、deadline到期、所属会话断连 | 缓冲归零 | 待复核 |
...
首轮复核FAIL：发现同会话第二个或重复`begin`可并存/覆盖，且输入拒绝与unknown未单独取证。修复提交完成后须由非实现者重新运行六行矩阵；通过后才能把AD-CX-02从Proposed更新为Accepted，并另建产品协议Change。
