...
- **WHEN** 原生setter已报告写入开始且SDK输入尚未返回
- **THEN** Harness请求shutdown，并等待原生应用与新SDK Observe及目标状态匹配后才记录通过
...
- **WHEN** 隔离SDK宿主的Accessibility权限不足
- **THEN** Harness不派发输入、不请求权限、不采集截图或正文，并记录门禁失败
...
#### Scenario: 动作间停止后拒绝新输入

- **GIVEN** 已确认唯一PID、窗口与AX字段
- **WHEN** 后台输入后Observe与目标状态匹配且shutdown完成
- **THEN** 第二次输入被拒绝，新SDK Observe及750ms目标状态保持稳定
...
#### Scenario: 受监管SDK子进程停止

- **WHEN** 宿主监督停止SDK子进程或私有IPC断开
- **THEN** SDK记录就绪、退出、断连和恢复结果，超时判失败且不启动上游App
...
- **WHEN** 官方发布哈希、解包配额、签名或系统评估任一项失败
- **THEN** Harness不运行、不重签、不移除隔离属性，并记录拒绝原因
